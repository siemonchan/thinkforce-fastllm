...
struct mve_buffer_external *mve_buffer_ashmem_create_buffer_external5(struct mve_buffer_info *info)
{
...
void mve_buffer_ashmem_destroy_buffer_external5(struct mve_buffer_external *buffer)
{
    MVE_RSRC_MEM_CACHE_FREE(buffer, sizeof(struct mve_buffer_external));
}

bool mve_buffer_ashmem_map_physical_pages5(struct mve_buffer_external *buffer)
{
    uint32_t size;
    void *ptr;
    uint32_t num_pages;
...
bool mve_buffer_ashmem_unmap_physical_pages5(struct mve_buffer_external *buffer)
{
    if (NULL == buffer)
    {
        return false;
...
void mve_buffer_ashmem_set_owner5(struct mve_buffer_external *buffer,
