...
struct mve_queue *mve_queue_create5(uint32_t size)
{
...
void mve_queue_destroy5(struct mve_queue *queue)
{
    if (NULL != queue)
    {
        MVE_RSRC_MEM_VFREE(queue->buffer);
...
void mve_queue_interrupt5(struct mve_queue *queue)
{
    queue->interrupted = true;
    wake_up_interruptible(&queue->wait_queue);
}

bool mve_queue_interrupted5(struct mve_queue *queue)
{
    return queue->interrupted;
}

struct mve_base_event_header *mve_queue_wait_for_event5(struct mve_queue *queue, uint32_t timeout)
{
    ...
    event = mve_queue_create_event5(header.code, header.size);
    if (NULL == event)
    {
...
void mve_queue_add_event5(struct mve_queue *queue,
...
struct mve_base_event_header *mve_queue_create_event5(int code, int size)
{
    struct mve_base_event_header *event;
...
bool mve_queue_event_ready5(struct mve_queue *queue)
{
