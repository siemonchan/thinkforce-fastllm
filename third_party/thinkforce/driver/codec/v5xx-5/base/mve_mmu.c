...
static phys_addr_t get_l2_line_addr(struct mve_mmu_ctx *ctx, mve_addr_t mve_addr)
{
    ...
    mve_mmu_entry_t l1_entry = (mve_mmu_entry_t)mve_rsrc_mem_read325(l1_entry_addr);
    ...
    if (0 == l1_entry)
    {
        ...
        mve_rsrc_mem_write325(l1_entry_addr, l1_entry);
        mve_rsrc_mem_clean_cache_range5(l1_entry_addr, sizeof(mve_mmu_entry_t));
    }
...
struct mve_mmu_ctx *mve_mmu_create_ctx5(void)
{
                            ...
                            uint32_t num_pages)
{
    ...
    while (curr < end)
    {
        ...
        do
        {
            ...
            entry = mve_rsrc_mem_read325(l2_line);
        ...
        if (0 < curr - start)
        {
            while (start < curr)
            {
                ...
                addr = mve_mmu_entry_get_paddr(mve_rsrc_mem_read325(l2_line));
                void *pstart = mve_rsrc_mem_cpu_map_page5(addr);
                void *pptr;
                ...
                do
                {
                    ...
                    mve_mmu_entry_t entry = mve_rsrc_mem_read325(l2_line);
                    pptr = mve_rsrc_mem_cpu_map_page5(mve_mmu_entry_get_paddr(entry));
                    /* Unmap entry immediately. We only want to know if the pages are linear
                     * in the virtual address space. */
                    mve_rsrc_mem_cpu_unmap_page5(mve_mmu_entry_get_paddr(entry));
                ...
                /* Unmap pstart */
                mve_rsrc_mem_cpu_unmap_page5(addr);
                ...
                int err = mveemul_add_memory(ctx->mmu_id,
                                             mve_rsrc_mem_cpu_map_page5(addr),
                              ...
                              struct mve_mmu_mapping *entry)
{
    ...
    while (curr < end)
    {
        ...
        do
        {
            ...
            l2_entry = mve_rsrc_mem_read325(l2_line);
            curr += MVE_MMU_PAGE_SIZE;
        }
...
void mve_mmu_destroy_ctx5(struct mve_mmu_ctx *ctx)
{
    int i;
    ...
    list_for_each_safe(pos, next, &(ctx->mappings->list))
    {
        ...
        if (NULL == map->pages && 0 < map->num_pages)
        {
             ...
             * on unmap. */
            MVE_LOG_PRINT(&mve_rsrc_log5, MVE_LOG_ERROR, "MMU leaked region. addr=0x%X-0x%X.",
                          map->mve_addr,
    ...
    for (i = 0; i < NUM_L1_ENTRIES; ++i)
    {
        ...
        l1_entry = mve_rsrc_mem_read325(ctx->l1_page + i * sizeof(mve_mmu_entry_t));
        if (0 != l1_entry)
        {
...
bool mve_mmu_map_page_replace5(struct mve_mmu_ctx *ctx,
                              phys_addr_t l2_page,
                              mve_addr_t mve_addr)
{
...
bool mve_mmu_map_pages_merge5(struct mve_mmu_ctx *ctx,
                             phys_addr_t l2_page,
                             mve_addr_t mve_addr,
                             uint32_t num_pages)
{
    ...
    for (curr_addr = start_addr; curr_addr < end_addr; curr_addr += MVE_MMU_PAGE_SIZE)
    {
        ...
        l2_entry = mve_rsrc_mem_read325(addr);
        if (0 != l2_entry)
        {
    ...
    for (i = 0; i < num_pages; ++i)
    {
        ...
        entry = mve_rsrc_mem_read325(l2_page + i * sizeof(mve_mmu_entry_t));
        mve_rsrc_mem_write325(addr, entry);
        mve_rsrc_mem_clean_cache_range5(addr, sizeof(mve_mmu_entry_t));
    }
...
bool mve_mmu_map_pages5(struct mve_mmu_ctx *ctx,
                       phys_addr_t *pages,
                       mve_addr_t mve_addr,
                       uint32_t num_pages,
                       uint32_t num_sys_pages,
...
void mve_mmu_unmap_pages5(struct mve_mmu_ctx *ctx,
                         mve_addr_t mve_addr)
{
    uint32_t i;
    ...
    do
    {
        ...
        {
            mve_rsrc_mem_write325(addr, 0);
            addr += sizeof(mve_mmu_entry_t);
        }
        /* Flush the cache */
        mve_rsrc_mem_clean_cache_range5(start, to_clear_current_iteration * sizeof(mve_mmu_entry_t));
...
bool mve_mmu_map_pages_and_reserve5(struct mve_mmu_ctx *ctx,
                                   phys_addr_t *pages,
                                   ...
                                   bool free_pages_on_unmap)
{
    ...
    do
    {
        ...
        for (i = 0; i < max_pages && true == space_found && curr_addr < end_addr; ++i)
        {
            ...
            {
                space_found = mve_rsrc_mem_read325(addr) == 0;
            }
        ...
        if (false == space_found)
        {
            ...
            while (curr_addr < end_addr)
            {
                ...
                if (0 != addr)
                {
                    mve_mmu_entry_t entry = mve_rsrc_mem_read325(addr);
                    if (0 == entry)
                    {
                        break;
                    }
...
bool mve_mmu_map_info5(struct mve_mmu_ctx *ctx,
                      mve_addr_t mve_addr,
                      uint32_t *num_pages,
                      uint32_t *max_pages)
{
...
#ifdef EMULATOR
BullseyeCoverageOff
bool mve_mmu_map_resize5(struct mve_mmu_ctx *ctx,
    ...
    if (map->num_pages != num_pages)
    {
        ...
        for (i = new_pages_start; i < map->max_pages; ++i)
        {
            ...
            else
            {
                ...
                mve_mmu_entry_t entry = mve_rsrc_mem_read325(l2_line_addr);
                ...
                mve_rsrc_mem_write325(l2_line_addr, entry);
                mve_rsrc_mem_clean_cache_range5(l2_line_addr, sizeof(mve_mmu_entry_t));
            }
        }
...
phys_addr_t mve_mmu_get_id5(struct mve_mmu_ctx *ctx)
{
    if (NULL == ctx)
    {
        return 0;
...
phys_addr_t mve_mmu_get_l1_page5(struct mve_mmu_ctx *ctx)
{
    if (NULL == ctx)
    {
        return 0;
...
bool mve_mmu_read_buffer5(struct mve_mmu_ctx *ctx,
                         mve_addr_t src,
                         uint8_t *dst,
                         uint32_t size)
{
    while (size > 0)
    {
        ...
        l2_entry = (mve_mmu_entry_t)mve_rsrc_mem_read325(l2addr);
        ...
        mve_rsrc_mem_invalidate_cache_range5(paddr, copy_size);
        ptr = mve_rsrc_mem_cpu_map_page5(paddr);
        if (NULL != ptr)
        {
            memcpy(dst, ptr, copy_size);
            mve_rsrc_mem_cpu_unmap_page5(paddr);
        }
