...
bool mver_dvfs_register_session5(const mver_session_id session_id)
{
    ...
    mver_pm_request_frequency5(max_freq5);
...
{
    uint32_t old = mver_pm_read_frequency5();
    uint32_t new;
#if (1 == DVFS_DEBUG_MODE)
    bool do_burst = false;
#endif
    ...
    if (old != new)
    {
        mver_pm_request_frequency5(new);
#if (1 == DVFS_DEBUG_MODE)
...
{
    int old_freq = (int)mver_pm_read_frequency5();
    int new_freq;
    ...
    {
        mver_pm_request_frequency5(new_freq);
#if (1 == DVFS_DEBUG_MODE)
        atomic_long_inc(&changes_cnt);
#endif
    }
...
static void update_sessions(void)
{
    ...
    list_for_each_safe(entry, safe, &sessions)
    {
        ...
        {
#if 0
            MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_WARNING,
...
{
    MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_INFO, "DVFS polling thread started");
    ...
    MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_INFO, "DVFS polling thread finished");
    return 0;
}
                                 ...
                                 char *buf)
{
    ...
    struct list_head *entry;
    struct session *session;
    uint32_t freq = mver_pm_read_frequency5();
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_enabled5(struct device *dev, struct device_attribute *attr,
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_freq5(struct device *dev, struct device_attribute *attr,
                       const char *buf, size_t count)
{
    int failed;
    unsigned int freq;
    ...
    if (!failed)
    {
        mver_pm_request_frequency5((uint32_t)freq);
    }
    return (failed) ? failed : count;
}
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_poll_interval_ms5(struct device *dev,
                                   struct device_attribute *attr,
                                   const char *buf, size_t count)
{
    int failed;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_up_step_percent5(struct device *dev, struct device_attribute *attr,
                                  const char *buf, size_t count)
{
    int failed;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_down_step_percent5(struct device *dev, struct device_attribute *attr,
                                    const char *buf, size_t count)
{
    int failed;
...
static void sysfs_register_devices(struct device *dev)
{
    ...
    while (i--)
    {
        ...
        if (err < 0)
        {
            MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_ERROR,
                    ...
                    const mver_dvfs_get_session_status_fptr get_session_status_callback)
{
    if (!initialized)
    {
        ...
        max_freq = mver_pm_read_max_frequency5();
        up_step_freq = ratio(max_freq, UP_STEP_PERCENT);
        down_step_freq = ratio(max_freq, DOWN_STEP_PERCENT);
...
void mver_dvfs_request_max_frequency5(void)
{
    ...
    if (false != adjust)
    {
        mver_pm_request_frequency5(max_freq);
    }
}

#endif

EXPORT_SYMBOL(mver_dvfs_request_max_frequency5);
