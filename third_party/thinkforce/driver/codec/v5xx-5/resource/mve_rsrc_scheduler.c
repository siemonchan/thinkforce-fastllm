...
static void disable_scheduling(tCS *regs)
{
    ...
    mver_reg_write325(&regs->ENABLE, 0);
    do
    {
        val = mver_reg_read325(&regs->ENABLE);
    }
    while (0 != val);
}
...
{
    wmb();
    mver_reg_write325(&regs->ENABLE, 1);
}
...
static enum LSID jobqueue_currently_executing(void)
{
    ...
    regs = mver_reg_get_coresched_bank5();
    core_lsid = mver_reg_read325(&regs->CORELSID);
    mver_reg_put_coresched_bank5(&regs);
...
static bool session_is_executing(struct mve_session_entry *session)
{
    ...
    regs = mver_reg_get_coresched_bank5();
    job_queue = mver_reg_read325(&regs->JOBQUEUE);
    core_lsid = mver_reg_read325(&regs->CORELSID);
    mver_reg_put_coresched_bank5(&regs);
...
static bool jobqueue_enqueue_session(struct mve_session_entry *session)
{
    ...
    if (false != job_added)
    {
        MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_DEBUG, "Enqueueing session. session=%p, lsid=%u, jobqueue=0x%08X, corelsid=0x%X.",
                      session->session_id, lsid, job_queue, mver_reg_read325(&regs->CORELSID));
        mver_reg_write325(&regs->JOBQUEUE, job_queue);
    }
...
static void jobqueue_dequeue_lsid(tCS *regs, enum LSID lsid)
{
    ...
    mver_reg_write325(&regs->JOBQUEUE, new_job_queue);
}
                                            ...
                                            char *buf)
{
    ...
    for (i = 0; i < mve_rsrc_data5.nlsid; ++i)
    {
...
static void session_ram_preserve(uint32_t *session_ram, tCS *regs, int lsid)
{
    ...
    {
        session_ram[i] = mver_reg_read325(&regs->SESSIONRAM[offset + i]);
    }
}
                                 ...
                                 char *buf)
{
    ...
    num += snprintf(buf, PAGE_SIZE, "%d\n", mve_rsrc_data5.ncore);
    return num;
}

ssize_t sysfs_set_cores5(struct device *dev, struct device_attribute *attr,
                        const char *buf, size_t count)
{
    ...
    regs = mver_reg_get_coresched_bank5();
    phys_cores = mver_reg_read325(&regs->NCORES);
    mver_reg_put_coresched_bank5(&regs);
    ...
    /* Remember core count. */
    mve_rsrc_data5.ncore = num_cores;
...
void mver_scheduler_init5(struct device *dev)
{
    INIT_LIST_HEAD(&sessions);
    sema_init(&scheduler_sem, 1);
    queued_sessions = mver_circular_buffer_create5(64);

    mver_dvfs_init5(dev, get_session_status);
#if defined(CONFIG_SYSFS) && defined(_DEBUG)
    {
        int err;
        int i;
        ...
        for (i = 0; i < NELEMS(sysfs_files); ++i)
        {
            ...
            if (err < 0)
            {
                MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_ERROR, "Unable to create sysfs file.");
            }
        }
    }
#endif
...
void mver_scheduler_deinit5(struct device *dev)
{
    int sem_taken;

    mver_dvfs_deinit5(dev);
    /* Destroy queued sessions. */
    sem_taken = down_interruptible(&scheduler_sem);

    mver_circular_buffer_destroy5(queued_sessions);
...
static void switch_out_session(struct mve_session_entry *session)
{
    ...
    val = mver_reg_read325(&regs->LSID[lsid].ALLOC);
    WARN_ON(val == 0);
    if (0 != val)
    {
        int sem_taken;
        ...
        mver_reg_write325(&regs->LSID[lsid].SCHED, 0);

        ncore = mver_reg_read325(&regs->NCORES);

        /* Make sure all firmware memory writes have completed */
        do
        {
            /* Get the current mask of which core is running what */
            corelsid_mask =  mver_reg_read325(&regs->CORELSID);
        ...
        mver_reg_write325(&regs->LSID[lsid].ALLOC, 0);
        session_by_lsid[session->lsid] = NULL;
        session->lsid = NO_LSID;

        MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_DEBUG, "Switching out session. session=%p, lsid=%u, corelsid=0x%X.",
                      session->session_id, lsid, mver_reg_read325(&regs->CORELSID));
        ...
        if (SCHEDULE_STATE_SLEEP != state && 0 == session->enqueues)
        {
            ...
            mver_circular_buffer_add5(queued_sessions, session);
            session->enqueues++;
        }
...
static bool map_session(struct mve_session_entry *session, enum LSID lsid)
{
    ...
    WARN_ON(mver_reg_read325(&regs->LSID[lsid].ALLOC) != 0);

    if (false == session->secure)
    {
        mver_reg_write325(&regs->LSID[lsid].ALLOC, 1);
    }
    else
    {
        mver_reg_write325(&regs->LSID[lsid].ALLOC, 2);
    }
    ...
    mver_reg_write325(&regs->LSID[lsid].TERMINATE, 1);
    ntries = WAIT_FOR_TERMINATE_NTRIES;
    do
    {
        val = mver_reg_read325(&regs->LSID[lsid].TERMINATE);
    }
    while (0 != val && --ntries);
    ...
    mver_reg_write325(&regs->LSID[lsid].MMU_CTRL, session->mmu_ctrl);
    mver_reg_write325(&regs->LSID[lsid].CTRL, disallow_cores | (session->ncores << CORESCHED_LSID_CTRL_MAXCORES));

    mver_reg_write325(&regs->LSID[lsid].BUSATTR[0], mve_rsrc_data5.port_attributes[0]);
    mver_reg_write325(&regs->LSID[lsid].BUSATTR[1], mve_rsrc_data5.port_attributes[1]);
    mver_reg_write325(&regs->LSID[lsid].BUSATTR[2], mve_rsrc_data5.port_attributes[2]);
    mver_reg_write325(&regs->LSID[lsid].BUSATTR[3], mve_rsrc_data5.port_attributes[3]);

    mver_reg_write325(&regs->LSID[lsid].FLUSH_ALL, 0);

    /* Clear all interrupts */
    mver_reg_write325(&regs->LSID[lsid].IRQVE, 0);
    mver_reg_write325(&regs->LSID[lsid].IRQHOST, 0);
    wmb();
    mver_reg_write325(&regs->LSID[lsid].SCHED, 1);
...
static bool switch_in_session(struct mve_session_entry *session)
{
    ...
    MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_DEBUG, "Switching in session. session=%p, lsid=%u, enqueued=%u.", session->session_id, session->lsid, nr_enqueues);
...
static void switch_in_pending_sessions(void)
{
    ...
    num = mver_circular_buffer_get_num_entries5(queued_sessions);
    if (0 == num)
    {
        /* No pending sessions */
        return;
    ...
    do
    {
        mver_circular_buffer_peek5(queued_sessions, (void **)&session);
        res = switch_in_session(session);
        if (false != res)
        {
            mver_circular_buffer_remove5(queued_sessions, (void **)&session);
            session->enqueues--;
            mver_irq_signal_mve5(session->session_id);
        }
    }
...
static void stop_session(struct mve_session_entry *session)
{
    ...
    if (NO_LSID != lsid)
    {
        ...
        regs = mver_reg_get_coresched_bank5();
        if (0 != mver_reg_read325(&regs->LSID[lsid].ALLOC))
        {
            uint32_t val;
            int ntries;
            ...
            MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_DEBUG, "Session stopped. Switching out session. session=%p.", session->session_id);
        }
...
bool mver_scheduler_register_session5(mver_session_id session_id,
                                     uint32_t mmu_ctrl,
                                     int ncores,
                                     bool secure,
                                     ...
                                     session_get_restricting_buffer_count_fptr get_restricting_buffer_count_callback)
{
    ...
    mver_dvfs_register_session5(session_id);
...
void mver_scheduler_unregister_session5(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    int sem_taken = 0;
...
//    sem_taken = down_interruptible(&scheduler_sem);
    ...
    mver_dvfs_unregister_session5(session_id);
    entry = get_session_cache_entry(session_id);
    if (NULL != entry)
    {
...
static void process_session_irq(struct mve_session_entry *entry)
{
    ...
    else if (SCHEDULE_STATE_RESCHEDULE == state && 0 == entry->enqueues)
    {
        ...
        if (false == is_executing)
        {
            ...
            mver_circular_buffer_add5(queued_sessions, entry);
            entry->enqueues++;
        }
    }
...
bool mver_scheduler_execute5(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    int sem_taken;
    bool ret = false;
    ...
    if (NULL != entry)
    {
        ...
        if (NO_LSID == entry->lsid)
        {
            ...
            else
            {
                ...
                mver_irq_signal_mve5(session_id);
                ret = true;
            }
...
void mver_scheduler_stop5(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    int sem_taken = 0;
...
//    sem_taken = down_interruptible(&scheduler_sem);
    ...
    if (NULL != entry)
    {
        mver_circular_buffer_remove_all_occurences5(queued_sessions, entry);
        stop_session(entry);
    }
...
void mver_scheduler_handle_irq5(int lsid)
{
    struct mve_session_entry *entry;
    int sem_taken;
...
void mver_scheduler_flush_tlb5(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    ...
    if (NULL != entry)
    {
        ...
        if (NO_LSID != lsid)
        {
            tCS *regs = mver_reg_get_coresched_bank5();
            mver_reg_write325(&regs->LSID[lsid].FLUSH_ALL, 0);
            mver_reg_put_coresched_bank5(&regs);
        }
    }
...
enum LSID mver_scheduler_get_session_lsid5(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    enum LSID lsid = NO_LSID;
...
void mver_scheduler_suspend5(void)
{
    int sem_taken;
    int i;
    bool done = false;
    ...
    MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_INFO, "All sessions suspended.");
}

void mver_scheduler_resume5(void)
{
    ...
    MVE_LOG_PRINT(&mve_rsrc_log_scheduler5, MVE_LOG_INFO, "Sessions resumed.");
}

void mver_scheduler_lock5()
{
    down(&scheduler_sem);
}

void mver_scheduler_unlock5()
{
    up(&scheduler_sem);
}

int mver_scheduler_get_ncores5(void)
{
#ifdef EMULATOR
    return 1;
#else
    return mve_rsrc_data5.ncore;
#endif
}

EXPORT_SYMBOL(mver_scheduler_register_session5);
EXPORT_SYMBOL(mver_scheduler_unregister_session5);
EXPORT_SYMBOL(mver_scheduler_execute5);
EXPORT_SYMBOL(mver_scheduler_stop5);
EXPORT_SYMBOL(mver_scheduler_flush_tlb5);
EXPORT_SYMBOL(mver_scheduler_lock5);
EXPORT_SYMBOL(mver_scheduler_unlock5);
EXPORT_SYMBOL(mver_scheduler_get_ncores5);
