 ...
 */
void mver_irq_handler_bottom5(struct work_struct *work)
{
    ...
    for (lsid = 0; lsid < mve_rsrc_data5.nlsid; lsid++)
    {
        ...
        if (irq_set != 0)
        {
            ...
            mdelay(irq_delay);
#endif
            mver_scheduler_handle_irq5(lsid);
        }
    }
}
 ...
 */
irqreturn_t mver_irq_handler_top5(int irq, void *dev_id)
{
    uint32_t irq_vector;
    tCS *regs;
    unsigned lsid;
    ...
    regs = mver_reg_get_coresched_bank_irq5();
    irq_vector = mver_reg_read325(&regs->IRQVE);
...
void mver_irq_handler_init5(struct device *dev)
{
#if defined(CONFIG_SYSFS) && defined(_DEBUG)
    int err;
#endif
    bottom_half.irq = 0;
    INIT_WORK(&bottom_half.work, mver_irq_handler_bottom5);

    mver_work_queue = (struct workqueue_struct *)create_workqueue("mv500-5-wq");
...
#ifdef UNIT
    mve_rsrc_data5.interrupts = 0;
#endif
}

void mver_irq_handler_deinit5(struct device *dev)
{
...
void mver_irq_signal_mve5(mver_session_id session)
{
    if (false != mve_rsrc_data5.hw_interaction)
    {
        tCS *regs = mver_reg_get_coresched_bank5();
        enum LSID lsid;

        lsid = mver_scheduler_get_session_lsid5(session);

        if (lsid != NO_LSID)
        {
            mver_reg_write325(&regs->LSID[lsid].IRQHOST, 1);
        }
...
int mver_irq_enable5(void)
{
    if (-1 != mve_rsrc_data5.irq_nr && 0 == mve_rsrc_data5.irq_enable_count++)
    {
        int irq_trigger = mve_rsrc_data5.irq_flags & IRQF_TRIGGER_MASK;
        int ret = request_irq(mve_rsrc_data5.irq_nr, mver_irq_handler_top5,
                              IRQF_SHARED | irq_trigger,
                              MVE_RSRC_DRIVER_NAME, &mve_rsrc_data5);
        if (0 != ret)
        {
            mve_rsrc_data5.irq_enable_count--;
            return -ENXIO;
        }

#ifdef INTERRUPT_POLLING
        {
            char name[] = "mve500-5-irq-thread";
            mve_rsrc_data5.irq_thread = kthread_run(mver_irq_thread_func, NULL, name); 
        }
#endif
    }
...
void mver_irq_disable5(void)
{
    if (-1 != mve_rsrc_data5.irq_nr && 1 == mve_rsrc_data5.irq_enable_count--)
    {
        free_irq(mve_rsrc_data5.irq_nr, &mve_rsrc_data5);
        cancel_work_sync(&bottom_half.work);
    }
}

EXPORT_SYMBOL(mver_irq_signal_mve5);
EXPORT_SYMBOL(mver_irq_enable5);
EXPORT_SYMBOL(mver_irq_disable5);
