...
void mver_pm_request_frequency5(uint32_t freq)
{
    ...
    dvfs_conf = (struct mve_dvfs_callback_conf *)mve_config_get_value5(mve_rsrc_data5.config,
                                                                      MVE_CONFIG_DEVICE_ATTR_DVFS_CALLBACKS);
    if (NULL == dvfs_conf || NULL == dvfs_conf->set_rate)
    {
        MVE_LOG_PRINT(&mve_rsrc_log5, MVE_LOG_ERROR, "Unable to change the frequency.");
    }
    else
    {
...
uint32_t mver_pm_read_frequency5(void)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log5, MVE_LOG_ERROR, "Unable to read the frequency.");
        freq = 0;
    }
    else
    {
                                    ...
                                    size_t count)
{
    ...
    if (0 != freq)
    {
        mver_pm_request_frequency5(freq);
    }
...
void mver_pm_deinit5(struct device *dev)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log5, MVE_LOG_ERROR, "no dvfs_conf or stop.");
    }
    else
    {
        dvfs_conf->stop();
...
{
    mver_irq_disable5();
    return pm_runtime_put_sync(&mve_rsrc_data5.pdev->dev);
}
...
int mver_pm_request_resume5(void)
{
    ...
    ret = pm_runtime_get_sync(&mve_rsrc_data5.pdev->dev);
    val = mver_irq_enable5();
    if (0 != val)
    {
        MVE_LOG_PRINT(&mve_rsrc_log5, MVE_LOG_ERROR, "Unable to register interrupt handler: \'%d\'", val);
    }

    return ret;
}

EXPORT_SYMBOL(mver_pm_request_frequency5);
EXPORT_SYMBOL(mver_pm_read_frequency5);
EXPORT_SYMBOL(mver_pm_request_suspend5);
EXPORT_SYMBOL(mver_pm_request_resume5);
...
int mver_pm_poweron5(void)
{
    ...
    pm_conf = (struct mve_pm_callback_conf *)mve_config_get_value5(mve_rsrc_data5.config,
...
{
    return mver_pm_suspend5(dev);
}

int mver_pm_runtime_resume5(struct device *dev)
{
    return mver_pm_resume5(dev);
}
