...
void mve_rsrc_mem_cache_init2(void)
{
...
void mve_rsrc_mem_cache_deinit2(void)
{
...
static struct kmem_cache *mve_rsrc_get_cachep(uint32_t size)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log2, MVE_LOG_ERROR, "No allocation slab found. size=%u.", size);
    }

    return cachep;
}

void *mve_rsrc_mem_cache_alloc2(uint32_t size, uint32_t flags)
{
    void *ptr;
...
void mve_rsrc_mem_cache_free2(void *ptr, uint32_t size)
{
    struct kmem_cache *cachep;
...
EXPORT_SYMBOL(mve_rsrc_mem_cache_alloc2);
EXPORT_SYMBOL(mve_rsrc_mem_cache_free2);
