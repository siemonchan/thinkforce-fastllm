...
static bool map_external_buffer_mve(struct mve_session *session,
                                    ...
                                    uint32_t alignment)
{
    return mve_mmu_map_pages_and_reserve6(session->mmu_ctx,
...
{
    mve_mmu_unmap_pages6(session->mmu_ctx, buffer->mapping.region.start);
                                                         ...
                                                         struct mve_buffer_info *info)
{
    ...
    buffer = mve_buffer_create_buffer_external6(info, port_index);
    if (NULL == buffer)
    {
        return NULL;
    }

    mve_mem_virt_region_get6(region_type, &buffer->mapping.region);

    res = mve_buffer_map_physical_pages6(buffer);
    if (false == res)
    {
        mve_buffer_destroy_buffer_external6(buffer);
        return NULL;
    ...
    else
    {
        if (false != info->do_cache_maintenance)
        {
            ...
            mve_buffer_set_owner6(buffer, MVE_BUFFER_OWNER_CPU, port_index);
        }
                                                    ...
                                                    struct mve_base_buffer_userspace *userspace_buffer)
{
    ...
    buffer = mve_session_buffer_get_external_descriptor6(session, userspace_buffer->buffer_id);
    if (NULL != buffer)
    {
    ...
    res = mve_buffer_is_valid6(&buffer_info) &&
          (0 != crc_info.handle ? mve_buffer_is_valid6(&crc_info) : true);
    if (false == res)
    {
#ifdef _DEBUG
        WARN_ON(true);
                                                      ...
                                                      mve_base_buffer_handle_t buffer_id)
{
    ...
    buffer = mve_session_buffer_get_external_descriptor6(session, buffer_id);
    if (NULL == buffer)
    {
                                                   ...
                                                   bool empty_this_buffer)
{
    ...
    buffer_client = mve_session_buffer_get_external_descriptor6(session, param->buffer_id);
        ...
        0 != param->nRegions)
    {
        ...
        ret = mve_com_add_input_buffer6(session, &roi_buf, MVE_COM_BUFFER_TYPE_ROI);
    ...
    {
        mve_buffer_set_owner6(buffer_client->buffer, MVE_BUFFER_OWNER_DEVICE, buffer_client->port_index);
        if (NULL != buffer_client->crc)
        {
            mve_buffer_set_owner6(buffer_client->crc, MVE_BUFFER_OWNER_DEVICE, buffer_client->port_index);
        }
    }

    if (false == empty_this_buffer)
    {
        ret = mve_com_add_output_buffer6(session, &buf, buffer_type);
    }
    else
    {
        ret = mve_com_add_input_buffer6(session, &buf, buffer_type);
    }
                                                   ...
                                                   mve_com_buffer *buffer)
{
    ...
    buffer_client = mve_session_buffer_get_external_descriptor6(session, buffer_id);
    if (NULL == buffer_client)
    {
        /* The supplied user-space allocated buffer has not been registered
         * or is invalid. */
    ...
    if (false != buffer_info->do_cache_maintenance)
    {
         ...
         * call handles cache management */
        mve_buffer_set_owner6(buffer_client->buffer, MVE_BUFFER_OWNER_CPU, buffer_client->port_index);
        if (NULL != buffer_client->crc)
        {
            mve_buffer_set_owner6(buffer_client->crc, MVE_BUFFER_OWNER_CPU, buffer_client->port_index);
        }
    }
                                                                    ...
                                                                    mve_base_buffer_handle_t buffer_id)
{
    ...
    return mve_com_add_message6(session, MVE_REQUEST_CODE_RELEASE_REF_FRAME, sizeof(data), (uint32_t *)&data);
}
