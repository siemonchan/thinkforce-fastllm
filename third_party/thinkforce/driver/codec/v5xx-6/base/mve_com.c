...
mve_base_error mve_com_add_message6(struct mve_session *session,
                                   uint16_t code,
                                   uint16_t size,
                                   uint32_t *data)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log_fwif6, MVE_LOG_ERROR, "%p mve_com_add_message() returned error status. ret=%u, code=%u, size=%u.", session, ret, code, size);
    }
...
uint32_t *mve_com_get_message6(struct mve_session *session,
...
mve_base_error mve_com_add_input_buffer6(struct mve_session *session,
                                        ...
                                        enum mve_com_buffer_type type)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log_fwif6, MVE_LOG_ERROR, "%p mve_com_add_input_buffer6() returned error status. ret=%u.", session, res);
    }

    return res;
}

mve_base_error mve_com_add_output_buffer6(struct mve_session *session,
                                         mve_com_buffer *buffer,
                                         enum mve_com_buffer_type type)
{
    mve_base_error res;
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log_fwif6, MVE_LOG_ERROR, "%p mve_com_add_output_buffer6() returned error status. ret=%u.", session, res);
    }

    return res;
}

mve_base_error mve_com_get_input_buffer6(struct mve_session *session,
                                        mve_com_buffer *buffer)
{
    mve_base_error ret;
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log_fwif6, MVE_LOG_ERROR, "%p mve_com_get_input_buffer6() returned error status. ret=%u.", session, ret);
    }

    return ret;
}

mve_base_error mve_com_get_output_buffer6(struct mve_session *session,
                                         mve_com_buffer *buffer)
{
    mve_base_error ret;
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log_fwif6, MVE_LOG_ERROR, "%p mve_com_get_output_buffer6() returned error status. ret=%u.", session, ret);
    }

    return ret;
}

mve_base_error mve_com_get_rpc_message6(struct mve_session *session,
                                       mve_com_rpc *rpc)
{
...
mve_base_error mve_com_put_rpc_message6(struct mve_session *session,
                                       mve_com_rpc *rpc)
{
...
void mve_com_delete6(struct mve_session *session)
{
    MVE_RSRC_MEM_FREE(session->com);
    session->com = NULL;
}

mve_base_error mve_com_set_interface_version6(struct mve_session *session,
                                             enum mve_base_fw_major_prot_ver version)
{
    ...
    switch (version)
    {
        ...
        {
            session->com = mve_com_host_interface_v2_new6();
            break;
        }

        case MVE_BASE_FW_MAJOR_PROT_V3:
        {
            session->com = mve_com_host_interface_v3_new6();
            break;
        }

        default:
        {
            MVE_LOG_PRINT(&mve_rsrc_log_fwif6, MVE_LOG_ERROR, "%p unsupported interface version configured. version=%u.", session, version);
            return MVE_BASE_ERROR_BAD_PARAMETER;
        }
    }
