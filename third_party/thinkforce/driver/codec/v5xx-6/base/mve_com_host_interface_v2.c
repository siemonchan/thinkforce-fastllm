                        ...
                        void *data)
{
    ...
    MVE_LOG_DATA(&mve_rsrc_log_fwif6, MVE_LOG_INFO, vec, vecs);
}
                                         ...
                                         uint32_t size)
{
    ...
    host_area = mve_rsrc_dma_mem_map6(host);
    mve_area = mve_rsrc_dma_mem_map6(mve);
    ...
    mve_rsrc_dma_mem_invalidate_cache6(mve);
    ...
    *(uint32_t *)msg_header = read_32(mve_area, &rpos);
    ...
    mve_rsrc_mem_flush_write_buffer6();
    ...
    MVE_LOG_EXECUTE(&mve_rsrc_log_fwif6, MVE_LOG_INFO,
                    log_message(session, channel, MVE_LOG_FWIF_DIRECTION_FIRMWARE_TO_HOST, msg_header, data));

out:
    mve_rsrc_dma_mem_clean_cache6(host);
    mve_rsrc_dma_mem_unmap6(host);
    mve_rsrc_dma_mem_unmap6(mve);
                                         ...
                                         struct mve_com_buffer_frame *frame)
{
    ...
    switch (frame->format)
    {
        ...
        case MVE_BASE_BUFFER_FORMAT_ABGR_8888:
            ...
            break;
        default:
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session6, MVE_LOG_ERROR, session, "Illegal com buffer format. format=%u", frame->format);
                                   ...
                                   size_t size)
{
    ...
    if (header->size != size)
    {
        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session6, MVE_LOG_WARNING, session, "Com message size did not match expected size. code=%u, size=%u, expected=%u.",
    ...
    *out = MVE_RSRC_MEM_CACHE_ALLOC(size, GFP_KERNEL);
    if (*out == NULL)
    {
        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session6, MVE_LOG_WARNING, session, "Failed to allocate memory for com message.");
        header->size = 0;
        return;
    }
                             ...
                             struct mve_msg_header *header)
{
    ...
    switch (header->code)
    {
        ...
        case MVE_RESPONSE_CODE_EVENT:
        {
            ...
            switch (response_v2.event.event_code)
            {
                ...
                case MVE_EVENT_PROCESSED:
                {
                    ...
                    {
                        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session6, MVE_LOG_ERROR, session, "Com event processed message should have size 0 or %u. size=%u.",
                ...
                default:
                {
                    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session6, MVE_LOG_ERROR, session, "Received com message with illegal event code. code=%u.", response_v2.event.event_code);
                    break;
                }
            }
            break;
        ...
        default:
        {
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session6, MVE_LOG_ERROR, session, "Received com message with illegal code. code=%u.", header->code);
            break;
        }
    }
                                      ...
                                      mve_com_rpc *rpc)
{
    ...
    rpc_area = mve_rsrc_dma_mem_map6(session->rpc_area);
    if (NULL == rpc_area)
    {
        return MVE_BASE_ERROR_INSUFFICIENT_RESOURCES;
    }

    mve_rsrc_dma_mem_invalidate_cache6(session->rpc_area);
    ...
    mve_rsrc_dma_mem_unmap6(session->rpc_area);
                                      ...
                                      mve_com_rpc *rpc)
{
    ...
    wmb();
    mve_rsrc_dma_mem_clean_cache6(session->rpc_area);
...
struct mve_com *mve_com_host_interface_v2_new6(void)
{
    ...
    if (com == NULL)
    {
        MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_WARNING, "Failed to allocate com object.");
        return NULL;
    }
...
const struct mve_mem_virt_region *mve_com_hi_v2_get_mem_virt_region6(void)
{
    return mem_regions;
}
