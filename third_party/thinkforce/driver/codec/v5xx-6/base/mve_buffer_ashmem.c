...
struct mve_buffer_external *mve_buffer_ashmem_create_buffer_external6(struct mve_buffer_info *info)
{
...
void mve_buffer_ashmem_destroy_buffer_external6(struct mve_buffer_external *buffer)
{
    MVE_RSRC_MEM_CACHE_FREE(buffer, sizeof(struct mve_buffer_external));
}

bool mve_buffer_ashmem_map_physical_pages6(struct mve_buffer_external *buffer)
{
    uint32_t size;
    void *ptr;
    uint32_t num_pages;
    ...
    pages = mve_rsrc_mem_map_virt_to_phys6(ptr - offset, size, buffer->mapping.write);
    if (NULL == pages)
    {
        return false;
    }
...
bool mve_buffer_ashmem_unmap_physical_pages6(struct mve_buffer_external *buffer)
{
    if (NULL == buffer)
    {
        return false;
     ...
     * buffer->pages. */
    mve_rsrc_mem_unmap_virt_to_phys6(buffer->mapping.pages, buffer->mapping.num_pages);
...
void mve_buffer_ashmem_set_owner6(struct mve_buffer_external *buffer,
                                 ...
                                 enum mve_port_index port)
{
    ...
    for (i = 0; i < buffer->mapping.num_pages; ++i)
    {
        ...
        sync_func(&mve_rsrc_data6.pdev->dev, buffer->mapping.pages[i] + offset, size, direction);
    }
}
