...
struct mve_rsrc_dma_mem_t *mve_rsrc_dma_mem_alloc_uncached6(uint32_t size)
{
    ...
    if (NULL != mem)
    {
        ...
        mem->cpu_addr = dma_alloc_coherent(&mve_rsrc_data6.pdev->dev, size, &mem->dma_handle, GFP_KERNEL);
...
void mve_rsrc_dma_mem_free_uncached6(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;

    dma_free_coherent(&mve_rsrc_data6.pdev->dev, ptr->header.size, ptr->cpu_addr, ptr->dma_handle);
    if (NULL != ptr->pages)
    {
...
void mve_rsrc_dma_mem_clean_cache_uncached6(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
    mve_rsrc_mem_flush_write_buffer6();
}

void mve_rsrc_dma_mem_invalidate_cache_uncached6(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
    mve_rsrc_mem_flush_write_buffer6();
}

void *mve_rsrc_dma_mem_map_uncached6(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;
...
void mve_rsrc_dma_mem_unmap_unchached6(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
}

phys_addr_t *mve_rsrc_dma_mem_get_pages_uncached6(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;
