...
static struct kmem_cache *mve_rsrc_get_cachep(uint32_t size)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_ERROR, "No allocation slab found. size=%u.", size);
    }
...
EXPORT_SYMBOL(mve_rsrc_mem_cache_alloc6);
EXPORT_SYMBOL(mve_rsrc_mem_cache_free6);
