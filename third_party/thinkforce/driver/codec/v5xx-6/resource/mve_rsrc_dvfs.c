...
uint32_t max_freq6 = 0;

void mver_dvfs_init6(struct device *dev,
                    const mver_dvfs_get_session_status_fptr get_session_status_callback)
{
    max_freq6 = mver_pm_read_max_frequency6();
}

void mver_dvfs_deinit6(struct device *dev)
{
    /* Nothing to do */
}

bool mver_dvfs_register_session6(const mver_session_id session_id)
{
    /* Request the maximum frequency. It's then up to the integration layer to
     * decide which frequency is actually used */
    mver_pm_request_frequency6(max_freq6);

    return true;
}

void mver_dvfs_unregister_session6(const mver_session_id session_id)
{
    /* Nothing to do */
}

void mver_dvfs_request_max_frequency6(void)
{
    /* Nothing to do */
}
...
{
    uint32_t old = mver_pm_read_frequency6();
    uint32_t new;
#if (1 == DVFS_DEBUG_MODE)
    bool do_burst = false;
#endif
    ...
    if (old != new)
    {
        mver_pm_request_frequency6(new);
#if (1 == DVFS_DEBUG_MODE)
...
{
    int old_freq = (int)mver_pm_read_frequency6();
    int new_freq;
    ...
    {
        mver_pm_request_frequency6(new_freq);
#if (1 == DVFS_DEBUG_MODE)
        atomic_long_inc(&changes_cnt);
#endif
    }
...
static void update_sessions(void)
{
    ...
    list_for_each_safe(entry, safe, &sessions)
    {
        ...
        {
#if 0
            MVE_LOG_PRINT(&mve_rsrc_log_scheduler6, MVE_LOG_WARNING,
...
{
    MVE_LOG_PRINT(&mve_rsrc_log_scheduler6, MVE_LOG_INFO, "DVFS polling thread started");
    ...
    MVE_LOG_PRINT(&mve_rsrc_log_scheduler6, MVE_LOG_INFO, "DVFS polling thread finished");
    return 0;
}
                                 ...
                                 char *buf)
{
    ...
    struct list_head *entry;
    struct session *session;
    uint32_t freq = mver_pm_read_frequency6();
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_enabled6(struct device *dev, struct device_attribute *attr,
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_freq6(struct device *dev, struct device_attribute *attr,
                       const char *buf, size_t count)
{
    int failed;
    unsigned int freq;
    ...
    if (!failed)
    {
        mver_pm_request_frequency6((uint32_t)freq);
    }
    return (failed) ? failed : count;
}
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_poll_interval_ms6(struct device *dev,
                                   struct device_attribute *attr,
                                   const char *buf, size_t count)
{
    int failed;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_up_step_percent6(struct device *dev, struct device_attribute *attr,
                                  const char *buf, size_t count)
{
    int failed;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_down_step_percent6(struct device *dev, struct device_attribute *attr,
                                    const char *buf, size_t count)
{
    int failed;
...
static void sysfs_register_devices(struct device *dev)
{
    ...
    while (i--)
    {
        ...
        if (err < 0)
        {
            MVE_LOG_PRINT(&mve_rsrc_log_scheduler6, MVE_LOG_ERROR,
                    ...
                    const mver_dvfs_get_session_status_fptr get_session_status_callback)
{
    if (!initialized)
    {
        ...
        max_freq = mver_pm_read_max_frequency6();
        up_step_freq = ratio(max_freq, UP_STEP_PERCENT);
        down_step_freq = ratio(max_freq, DOWN_STEP_PERCENT);
...
void mver_dvfs_request_max_frequency6(void)
{
    ...
    if (false != adjust)
    {
        mver_pm_request_frequency6(max_freq);
    }
}

#endif

EXPORT_SYMBOL(mver_dvfs_request_max_frequency6);
