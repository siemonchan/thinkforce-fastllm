...
{
    MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_INFO, "Enable power.");
...
{
    MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_INFO, "Disable power.");
    /* No need to do anything on the FPGA */
}
...
static void dvfs_cores_pause(void)
{
    ...
    if (req != val)
    {
        MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_WARNING,
...
{
    MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_INFO, "Enable clock.");
}

static void disable_clock(void)
{
    MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_INFO, "Disable clock.");
}

static void set_clock_rate(uint32_t clk_rate)
{
    MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_DEBUG, "Setting clock rate. rate=%u (%u)", clk_rate, dvfs_freq);
#if defined(ENABLE_DVFS_FREQ_SIM)
    dvfs_sim_init();
...
static void stop(void)
{
    ...
    if (dvfs_is_started)
    {
        MVE_LOG_PRINT(&mve_rsrc_log6, MVE_LOG_DEBUG, "Stopping thread.");
...
struct mve_config_attribute *mve_device_get_config6(void)
{
    return attributes;
}
