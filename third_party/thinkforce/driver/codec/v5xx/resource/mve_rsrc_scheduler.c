...
void mver_scheduler_init(struct device *dev)
{
    ...
    queued_sessions = mver_circular_buffer_create(128);
...
void mver_scheduler_lock()
{
    down(&scheduler_sem);
}

void mver_scheduler_unlock()
{
    up(&scheduler_sem);
}
...
EXPORT_SYMBOL(mver_scheduler_lock);
EXPORT_SYMBOL(mver_scheduler_unlock);
