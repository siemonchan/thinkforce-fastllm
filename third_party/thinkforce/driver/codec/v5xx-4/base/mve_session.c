...
static void free_session(struct kref *ref)
{
    ...
    mve_session_buffer_unmap_all4(session);

    mve_queue_destroy4(session->response_events);
    mve_queue_destroy4(session->mve_events);

    mve_mem_virt_region_get4(VIRT_MEM_REGION_MSG_IN_QUEUE, &region);
    mve_mmu_unmap_pages4(session->mmu_ctx, region.start);

    mve_mem_virt_region_get4(VIRT_MEM_REGION_MSG_OUT_QUEUE, &region);
    mve_mmu_unmap_pages4(session->mmu_ctx, region.start);

    mve_mem_virt_region_get4(VIRT_MEM_REGION_INPUT_BUFFER_IN, &region);
    mve_mmu_unmap_pages4(session->mmu_ctx, region.start);

    mve_mem_virt_region_get4(VIRT_MEM_REGION_INPUT_BUFFER_OUT, &region);
    mve_mmu_unmap_pages4(session->mmu_ctx, region.start);

    mve_mem_virt_region_get4(VIRT_MEM_REGION_OUTPUT_BUFFER_IN, &region);
    mve_mmu_unmap_pages4(session->mmu_ctx, region.start);

    mve_mem_virt_region_get4(VIRT_MEM_REGION_OUTPUT_BUFFER_OUT, &region);
    mve_mmu_unmap_pages4(session->mmu_ctx, region.start);

    mve_mem_virt_region_get4(VIRT_MEM_REGION_RPC_QUEUE, &region);
    mve_mmu_unmap_pages4(session->mmu_ctx, region.start);

    mve_rsrc_dma_mem_free4(session->msg_in_queue);
    mve_rsrc_dma_mem_free4(session->msg_out_queue);
    mve_rsrc_dma_mem_free4(session->buf_input_in);
    mve_rsrc_dma_mem_free4(session->buf_input_out);
    mve_rsrc_dma_mem_free4(session->buf_output_in);
    mve_rsrc_dma_mem_free4(session->buf_output_out);
    mve_rsrc_dma_mem_free4(session->rpc_area);

    MVE_RSRC_MEM_FREE(session->role);
    ret = mve_fw_unload4(session->mmu_ctx, session->fw_inst);
    if (false == ret)
    {
        MVE_LOG_PRINT(&mve_rsrc_log_session4, MVE_LOG_DEBUG, "Firmware unload failed.");
    }
    mve_mmu_destroy_ctx4(session->mmu_ctx);
...
static bool change_session_state(struct mve_session *session, enum MVE_SESSION_STATE to_state)
{
    ...
    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_DEBUG, session, "Changing state. '%s' -> '%s'.", state_to_char[session->state.state], state_to_char[to_state]);
    ...
    {
        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_ERROR, session, "Invalid state change. '%s' -> '%s'.", state_to_char[session->state.state], state_to_char[to_state]);
    }
                                      ...
                                      void *data)
{
    ...
    ret = mve_com_add_message4(session, code, size, data);
...
{
    mve_queue_add_event4(session->mve_events, event);
#ifdef EMULATOR
    {
...
static void return_pending_buffer_to_userspace(struct mve_session *session, struct session_enqueue_request *request, bool empty_this_buffer)
{
    ...
    event = mve_queue_create_event4(event_code, sizeof(struct mve_base_buffer_userspace));
    if (NULL == event)
    {
        return;
    }
...
static mve_base_error enqueue_job(struct mve_session *session)
{
    ...
    cmd.cores = mver_scheduler_get_ncores4();
...
static void enqueue_pending_buffers(struct mve_session *session, struct list_head *list, bool empty_this_buffer)
{
    ...
    list_for_each_safe(pos, next, list)
    {
        ...
        ret = mve_session_buffer_enqueue_internal4(session, &request->param, empty_this_buffer);
...
static bool schedule_session(struct mve_session *session)
{
    ...
    {
        ret = mver_scheduler_execute4((mver_session_id)session);
    }
...
static void switchout_session(struct mve_session *session)
{
        ...
        STATE_SWITCHING_IN != session->state.state)
    {
        ...
        if (false != ret)
        {
            ...
            mver_irq_signal_mve4((mver_session_id)session);
        }
    }
...
{
    struct mve_base_fw_version *ver = mve_fw_get_version4(session->fw_inst);
    bool ret = false;
...
static void process_generic_event(struct mve_session *session, struct mve_base_event_header *event)
{
    ...
    switch (event->code)
    {
        ...
        case MVE_BASE_EVENT_INPUT:
        {
            ...
            {
                mve_com_get_output_buffer4(session, &buffer);
            }
            else
            {
                mve_com_get_input_buffer4(session, &buffer);
            }
            ...
            if (NULL != buffer_client)
            {
                ...
                else
                {
                    ...
                    res = mve_session_buffer_dequeue_internal4(session, buffer_client->buffer->info.buffer_id, &buffer);
                        ...
                        MVE_BASE_BUFFERFLAG_EOS == (buffer_client->flags & MVE_BASE_BUFFERFLAG_EOS))
                    {
                        ...
                        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_DEBUG, session, "Resetting eos_queued.");
                    }

                    usr_event = mve_queue_create_event4(event->code, sizeof(struct mve_base_buffer_userspace));
                    if (NULL == usr_event)
                    {
                        return;
                    }
                    ...
                    mve_session_buffer_convert_to_userspace4(buf, buffer_client);
            ...
            if (0 != (session->state.flush_target & MVE_BASE_FLUSH_QUICK))
            {
                    ...
                    session->state.flush_state)
                {
                    ...
                    list_for_each_safe(pos, next, &session->quick_flush_buffers)
                    {
                        ...
                        param.timestamp = 0;
                        ret = mve_session_buffer_enqueue_internal4(session,
                                                                  &param,
        ...
        case MVE_BASE_EVENT_FW_ERROR:
        {
            ...
            {
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_ERROR, session, "Unsupported firmware.");
                ...
                if (STATE_WAITING_FOR_RESPONSE == session->state.state)
                {
                    mve_queue_interrupt4(session->response_events);
                }
            }
            else if (NULL != e && MVE_ERROR_WATCHDOG == e->error_code && !strcmp(e->message, "Watchdog"))
            {
                ...
                host_area = mve_rsrc_dma_mem_map4(session->msg_in_queue);
                mve_area = mve_rsrc_dma_mem_map4(session->msg_out_queue);

                if (host_area != NULL && mve_area != NULL)
                {
                    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_ERROR, session, "host_area wpos: %d mve_area rpos: %d.", host_area->in_wpos, mve_area->in_rpos);
                }
                mve_rsrc_dma_mem_unmap4(session->msg_in_queue);
                mve_rsrc_dma_mem_unmap4(session->msg_out_queue);
            }
            break;
        }
        ...
        case MVE_BASE_EVENT_REF_FRAME_RELEASED:
        {
            ...
            if (NULL != buffer_client)
            {
                ...
                struct mve_base_event_header *usr_event;
                usr_event = mve_queue_create_event4(event->code, sizeof(mve_base_buffer_handle_t));
                if (NULL == usr_event)
                {
                    return;
                }
        ...
        default:
        {
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_ERROR, session, "Illegal event code. code=%u.", event->code);
            return;
        }
    }
...
static void handle_mve_message(struct mve_session *session)
{
    ...
    data = mve_com_get_message4(session, &header);
    while (NULL != data)
    {
        enum mve_base_event_code event_code;
        struct mve_base_event_header *event;
        ...
        event = mve_queue_create_event4(event_code, size);
        if (NULL != event)
        {
            ...
            if (MVE_BASE_EVENT_FW_ERROR == event->code)
            {
                ...
                MVE_LOG_PRINT(&mve_rsrc_log_session4, MVE_LOG_ERROR, "Firmware error. reason=%u, msg=%s.", err->reason, err->msg);
            }
            ...
            switch (event_code)
            {
                case MVE_BASE_EVENT_SET_PARAMCONFIG:
                    ...
                    response_received(session);
                    mve_queue_add_event4(session->response_events, event);
#ifdef EMULATOR
                    {
                        int t = (int)(((uintptr_t)session->filep) & 0xFFFFFFFF);
                        write(t, &t, sizeof(int));
...
mve_base_error mve_session_handle_rpc_mem_alloc4(struct mve_session *session, int fd)
{
    ...
    err = mve_com_get_rpc_message4(session, &rpc);
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        buffer = mve_buffer_create_buffer_external4(&info, 0);
        if (NULL != buffer)
        {
            bool ret = mve_buffer_map_physical_pages4(buffer);
        ...
        if (NULL != pages)
        {
            ...
            {
                mve_mem_virt_region_get4(VIRT_MEM_REGION_PROTECTED, &region);
            }
            else
            {
                mve_mem_virt_region_get4(VIRT_MEM_REGION_OUT_BUF, &region);
            }

            ret = mve_mmu_map_pages_and_reserve4(session->mmu_ctx,
                                                pages,
                                                &region,
                                                num_pages,
                                                max_pages,
            ...
            mve_buffer_unmap_physical_pages4(buffer);
            mve_buffer_destroy_buffer_external4(buffer);
            buffer = NULL;
        }

        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_DEBUG, session,
...
error:
    ...
    mve_com_put_rpc_message4(session, &rpc);
...
mve_base_error mve_session_handle_rpc_mem_resize4(struct mve_session *session, int fd)
{
    struct mve_com_rpc rpc;
    mve_base_error err;
    ...
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        ret = mve_mmu_map_info4(session->mmu_ctx,
                               mve_addr,
                               &mapped_pages,
                               &max_pages);
        ...
        if (NULL != buffer)
        {
            ret = mve_buffer_map_physical_pages4(buffer);
            if (false != ret && num_new_pages <= buffer->mapping.num_pages)
            {
                ret = mve_mmu_map_resize4(session->mmu_ctx,
...
error:
        mver_scheduler_flush_tlb4((mver_session_id)session);

        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_DEBUG, session, "RPC resize requested. new_size=%d, fd: %d, num_pages=%d, addr=0x%X ret_addr= %#x.", new_size, fd, num_pages, mve_addr, ret_addr);
...
static void handle_rpc_request(struct mve_session *session)
{
    ...
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        {
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_INFO, session, "<- RPC %s", print_rpc_code(rpc.call_id));
        }
        ...
        switch (rpc.call_id)
        {
            case MVE_COM_RPC_FUNCTION_DEBUG_PRINTF:
            {
                ...
                event = mve_queue_create_event4(MVE_BASE_EVENT_RPC_PRINT, size);
                if (NULL == event)
                {
                    return;
                }
            ...
            case MVE_COM_RPC_FUNCTION_MEM_ALLOC:
            {
                ...
                phys_addr_t *pages;
                uint32_t mve_addr = 0;
                bool secure = mve_fw_secure4(session->fw_inst);
...
//                num_sys_pages = (size + PAGE_SIZE - 1) / PAGE_SIZE;
                ...
                if (false != secure)
                {
                    ...
                    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_DEBUG, session, "%s send event to userspace size: %d", __FUNCTION__, num_pages * MVE_MMU_PAGE_SIZE);

                    event = mve_queue_create_event4(MVE_BASE_EVENT_RPC_MEM_ALLOC, sizeof(struct mve_base_rpc_memory));
                    if (NULL != event)
                    {
                ...
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_INFO, session,
            ...
            case MVE_COM_RPC_FUNCTION_MEM_RESIZE:
            {
                ...
                if (false != secure)
                {
                    ...
                    bool ret = mve_mmu_map_info4(session->mmu_ctx,
                                                mve_addr,
                                                &mapped_pages,
                                                &max_pages);
                    if (false == ret || num_pages > max_pages)
                    ...
                    if (num_pages > mapped_pages)
                    {
                        ...
                        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_DEBUG, session, "%s send event to userspace old: %d extra: %d",
                                              __FUNCTION__, mapped_pages * MVE_MMU_PAGE_SIZE, num_new_pages * MVE_MMU_PAGE_SIZE);

                        event = mve_queue_create_event4(MVE_BASE_EVENT_RPC_MEM_RESIZE, sizeof(struct mve_base_rpc_memory));
                        if (NULL != event)
                        {
                            rpc_memory = (struct mve_base_rpc_memory *)event->data;
                            rpc_memory->region = VIRT_MEM_REGION_PROTECTED == mve_mem_virt_region_type_get4(mve_addr) ?
                ...
                {
                    bool ret = mve_mmu_map_resize4(session->mmu_ctx,
                                                  NULL,
                                                  mve_addr,
                                                  num_pages,
                                                  ATTRIB_SHARED_RW,
...
resize_error:
                ...
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_INFO, session, "RPC resize requested. new_size=%d, num_pages=%d, mve_addr=0x%X, ret_addr: 0x%X.", new_size, num_pages, mve_addr, ret_addr);
            ...
            case MVE_RPC_FUNCTION_MEM_FREE:
            {
                ...
                {
                    mve_mmu_unmap_pages4(session->mmu_ctx, rpc.params.mem_free.ve_pointer);
                }

                mver_scheduler_flush_tlb4((mver_session_id)session);

                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_INFO, session, "RPC free requested. addr=0x%X.", rpc.params.mem_free.ve_pointer);
            ...
            default:
            {
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_ERROR, session, "Unsupported RPC request. id=%u.", rpc.call_id);
                rpc.size = 0;
            }
        }
 ...
 */
void mve_session_irq_callback4(mver_session_id session_id)
{
...
static void mve_session_hung(struct mve_session *session)
{
    ...
    event = mve_queue_create_event4(MVE_BASE_EVENT_SESSION_HUNG, 0);
    if (NULL == event)
    {
        return;
    }
    ...
    mver_scheduler_stop4((mver_session_id)session);
    mver_scheduler_unregister_session4((mver_session_id)session);
}
...
static int watchdog_thread_func(void *data)
{
    ...
    do
    {
       ...
       irq = mver_reg_read324(&regs->IRQVE);
       printk("%s : irq is 0x%x\n", __func__, irq);
...
static void session_destroy_instance(struct mve_session *session)
{
    ...
    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_INFO, session, "Session destroyed.");

    mver_scheduler_lock4();
    sem_taken = down_interruptible(&sessions_sem);
...
static void mve_session_switchin_callback(mver_session_id session_id)
{
    ...
    mver_irq_signal_mve4((mver_session_id)session_id);
...
static void mve_session_switchout_completed_callback(mver_session_id session_id)
{
    ...
    mve_session_irq_callback4(session_id);
...
#define TRACE_SIZE 128 * 1024
    ...
    if (NULL == str)
    {
        MVE_LOG_PRINT(&mve_rsrc_log_session4, MVE_LOG_ERROR, "Unable to allocate memory for FW trace buffer.");
        return -EBUSY;
    }
                ...
                if ((1 << rasc) & fw_trace->rasc_mask)
                {
                    ...
                    if (0 != rasc_addr)
                    {
                        ...
                        if (NULL != buffer)
                        {
                            ...
                            mve_mmu_read_buffer4(session->mmu_ctx, rasc_addr, (uint8_t *)buffer, size);
...
void mve_session_init4(struct device *dev)
{
...
#if defined(_DEBUG) && !defined(EMULATOR)
    {
        ...
        {
            MVE_LOG_PRINT(&mve_rsrc_log_session4, MVE_LOG_ERROR, "Unable to create debugfs file 'fw_trace'.");
        }
    }
...
#if defined(CONFIG_SYSFS)
    {
        ...
        for (i = 0; i < NELEMS(sysfs_files); ++i)
        {
            ...
            if (err < 0)
            {
                MVE_LOG_PRINT(&mve_rsrc_log_session4, MVE_LOG_ERROR, "Unable to create sysfs file.");
            }
        }
    }
#endif /* CONFIG_SYSFS */
...
void mve_session_deinit4(struct device *dev)
{
#ifndef EMULATOR
#if defined(CONFIG_SYSFS) && defined(_DEBUG)
    int i;
    ...
    mve_session_destroy_all_sessions4();
}

void mve_session_destroy_all_sessions4(void)
{
    struct list_head *pos, *next;
...
struct mve_session *mve_session_create4(struct file *filep)
{
    struct mve_session *session;
    struct mve_mem_virt_region region;
    bool ret;
    ...
    session->mmu_ctx = mve_mmu_create_ctx4();
    ...
    session->msg_in_queue   = mve_rsrc_dma_mem_alloc4(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->msg_out_queue  = mve_rsrc_dma_mem_alloc4(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_input_in   = mve_rsrc_dma_mem_alloc4(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_input_out  = mve_rsrc_dma_mem_alloc4(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_output_in  = mve_rsrc_dma_mem_alloc4(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_output_out = mve_rsrc_dma_mem_alloc4(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->rpc_area       = mve_rsrc_dma_mem_alloc4(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    ...
    mve_mem_virt_region_get4(VIRT_MEM_REGION_MSG_IN_QUEUE, &region);
    pages = mve_rsrc_dma_mem_get_pages4(session->msg_in_queue);
    ret = mve_mmu_map_pages4(session->mmu_ctx, pages,
    ...
    mve_mem_virt_region_get4(VIRT_MEM_REGION_MSG_OUT_QUEUE, &region);
    pages = mve_rsrc_dma_mem_get_pages4(session->msg_out_queue);
    ret = mve_mmu_map_pages4(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_mem_virt_region_get4(VIRT_MEM_REGION_INPUT_BUFFER_IN, &region);
    pages = mve_rsrc_dma_mem_get_pages4(session->buf_input_in);
    ret = mve_mmu_map_pages4(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_mem_virt_region_get4(VIRT_MEM_REGION_INPUT_BUFFER_OUT, &region);
    pages = mve_rsrc_dma_mem_get_pages4(session->buf_input_out);
    ret = mve_mmu_map_pages4(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_mem_virt_region_get4(VIRT_MEM_REGION_OUTPUT_BUFFER_IN, &region);
    pages = mve_rsrc_dma_mem_get_pages4(session->buf_output_in);
    ret = mve_mmu_map_pages4(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_mem_virt_region_get4(VIRT_MEM_REGION_OUTPUT_BUFFER_OUT, &region);
    pages = mve_rsrc_dma_mem_get_pages4(session->buf_output_out);
    ret = mve_mmu_map_pages4(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    /* Map RPC area */
    mve_mem_virt_region_get4(VIRT_MEM_REGION_RPC_QUEUE, &region);
    pages = mve_rsrc_dma_mem_get_pages4(session->rpc_area);
    ret = mve_mmu_map_pages4(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    session->mve_events = mve_queue_create4(16384);
    session->response_events = mve_queue_create4(8192);
    ...
    mver_pm_request_resume4();

#ifndef DISABLE_WATCHDOG
    watchdog_create_session();
#endif

    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_INFO, session, "Session created.");
...
void mve_session_destroy4(struct mve_session *session)
{
    bool ret;
    int sem_taken;
...
struct mve_session *mve_session_get_by_file4(struct file *filep)
{
    struct list_head *pos, *next;
    struct mve_session *session = NULL;
    int sem_taken;
...
void mve_session_cleanup_client4(struct file *filep)
{
    struct mve_session *session;
...
void mve_session_destroy_all4(void)
{
    struct list_head *pos, *next;
...
void mve_session_set_role4(struct mve_session *session, char *role)
{
    bool ret;
...
bool mve_session_activate4(struct mve_session *session, uint32_t *version,
                          struct mve_base_fw_secure_descriptor *fw_secure_desc)
{
    ...
    ncores = mver_scheduler_get_ncores4();
    ...
    session->fw_inst = mve_fw_load4(session->mmu_ctx, fw_secure_desc, session->role, ncores);
    if (NULL != session->fw_inst)
    {
        ...
        ver = mve_fw_get_version4(session->fw_inst);
        if (NULL != ver)
        {
        ...
        error = mve_com_set_interface_version4(session, session->prot_ver);
        mve_fw_log_fw_binary4(session->fw_inst, session);

        if (error == MVE_BASE_ERROR_NONE)
        {
            l1_page = mve_mmu_get_id4(session->mmu_ctx);
            mver_scheduler_register_session4((mver_session_id)session,
                                            mve_mmu_make_l0_entry(ATTRIB_PRIVATE, l1_page),
                                            ncores,
                                            mve_fw_secure4(session->fw_inst),
                                            mve_session_irq_callback4,
...
mve_base_error mve_session_enqueue_flush_buffers4(struct mve_session *session, uint32_t flush)
{
    bool res;
...
mve_base_error mve_session_enqueue_state_change4(struct mve_session *session, enum mve_base_hw_state state)
{
    bool res;
    mve_base_error ret = MVE_BASE_ERROR_NONE;
...
struct mve_base_event_header *mve_session_get_event4(struct mve_session *session, uint32_t timeout)
{
    ...
    if (true == session->fw_loaded)
    {
        event = mve_queue_wait_for_event4(session->mve_events, timeout);
    }
...
mve_base_error mve_session_set_paramconfig4(struct mve_session *session,
                                           uint32_t size,
                                           void *data,
                                           ...
                                           enum mve_base_command_type entity)
{
    ...
    if (MVE_BASE_ERROR_NONE == ret)
    {
        ...
        {
            unlock_session(session);
            event = mve_queue_wait_for_event4(session->response_events, 10000);
            lock_session(session);

            if (NULL == event)
            {
                if (false != mve_queue_interrupted4(session->response_events))
                {
                    ret = MVE_BASE_ERROR_HARDWARE;
                }
                else
...
mve_base_error mve_session_buffer_register4(struct mve_session *session,
                                           uint32_t port_index,
                                           struct mve_base_buffer_userspace *descriptor)
{
    ...
    mver_scheduler_lock4();
    lock_session(session);
    ret = mve_session_buffer_register_internal4(session, port_index, descriptor);
    unlock_session(session);
    mver_scheduler_unlock4();
    release_session(session);
...
mve_base_error mve_session_buffer_unregister4(struct mve_session *session,
                                             mve_base_buffer_handle_t buffer_id)
{
    ...
    mver_scheduler_lock4();
    lock_session(session);
    ret = mve_session_buffer_unregister_internal4(session, buffer_id);
    unlock_session(session);
    mver_scheduler_unlock4();
    release_session(session);
...
mve_base_error mve_session_buffer_enqueue4(struct mve_session *session,
                                          struct mve_base_buffer_details *param,
                                          bool empty_this_buffer)
{
    ...
    if (false != empty_this_buffer)
    {
        ...
        {
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session4, MVE_LOG_DEBUG, session, "Setting eos_queued.");
        }
    }
    else
    {
        ...
        (false == empty_this_buffer && (0 != (session->state.flush_target & MVE_BASE_FLUSH_OUTPUT_PORT))))
    {
        if (session->state.flush_target & MVE_BASE_FLUSH_QUICK)
        {
            struct mve_buffer_client *buffer_client = mve_session_buffer_get_external_descriptor4(session, param->buffer_id);
    ...
    ret = mve_session_buffer_enqueue_internal4(session, param, empty_this_buffer);
...
mve_base_error mve_session_buffer_notify_ref_frame_release4(struct mve_session *session,
                                                           mve_base_buffer_handle_t buffer_id)
{
    mve_base_error ret = MVE_BASE_ERROR_NONE;
    bool res;
    ...
    lock_session(session);
    mve_session_buffer_notify_ref_frame_release_internal4(session, buffer_id);
    unlock_session(session);
    res = schedule_session(session);
    if (false != res)
    {
...
mve_base_error mve_session_enqueue_message4(struct mve_session *session,
                                           uint32_t code,
                                           uint32_t size,
                                           void *data)
{
...
#ifndef EMULATOR
uint32_t mve_session_poll4(struct file *filp, struct poll_table_struct *poll_table)
{
    struct mve_session *session;
    uint32_t mask = 0;

    session = mve_session_get_by_file4(filp);
    if (NULL != session)
    {
        lock_session(session);
        ...
        if (false != mve_queue_event_ready4(session->mve_events))
        {
...
#ifdef UNIT
#ifndef DISABLE_WATCHDOG
void mve_session_firmware_hung_simulation4(struct mve_session *session, uint32_t on)
{
    if (1 == on)
    {
