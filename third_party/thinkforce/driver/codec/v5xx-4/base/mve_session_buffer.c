...
static bool map_external_buffer_mve(struct mve_session *session,
                                    ...
                                    uint32_t alignment)
{
    return mve_mmu_map_pages_and_reserve4(session->mmu_ctx,
...
{
    mve_mmu_unmap_pages4(session->mmu_ctx, buffer->mapping.region.start);
    mver_scheduler_flush_tlb4((mver_session_id)session);
}

struct mve_buffer_client *mve_session_buffer_get_external_descriptor4(struct mve_session *session,
...
void mve_session_buffer_unmap_all4(struct mve_session *session)
{
                                                         ...
                                                         struct mve_buffer_info *info)
{
    ...
    buffer = mve_buffer_create_buffer_external4(info, port_index);
    if (NULL == buffer)
    {
        return NULL;
    }

    mve_mem_virt_region_get4(region_type, &buffer->mapping.region);

    res = mve_buffer_map_physical_pages4(buffer);
    if (false == res)
    {
        mve_buffer_destroy_buffer_external4(buffer);
        return NULL;
    ...
    else
    {
        if (false != info->do_cache_maintenance)
        {
            ...
            mve_buffer_set_owner4(buffer, MVE_BUFFER_OWNER_CPU, port_index);
        }
...
mve_base_error mve_session_buffer_register_internal4(struct mve_session *session,
                                                    uint32_t port_index,
                                                    struct mve_base_buffer_userspace *userspace_buffer)
{
    ...
    buffer = mve_session_buffer_get_external_descriptor4(session, userspace_buffer->buffer_id);
    if (NULL != buffer)
    {
    ...
    res = mve_buffer_is_valid4(&buffer_info) &&
          (0 != crc_info.handle ? mve_buffer_is_valid4(&crc_info) : true);
    if (false == res)
    {
#ifdef _DEBUG
        WARN_ON(true);
...
mve_base_error mve_session_buffer_unregister_internal4(struct mve_session *session,
                                                      mve_base_buffer_handle_t buffer_id)
{
    struct mve_buffer_client *buffer;
    mve_base_error ret = MVE_BASE_ERROR_NONE;

    buffer = mve_session_buffer_get_external_descriptor4(session, buffer_id);
    if (NULL == buffer)
    {
...
mve_base_error mve_session_buffer_enqueue_internal4(struct mve_session *session,
                                                   ...
                                                   bool empty_this_buffer)
{
    ...
    buffer_client = mve_session_buffer_get_external_descriptor4(session, param->buffer_id);
        ...
        0 != param->nRegions)
    {
        ...
        ret = mve_com_add_input_buffer4(session, &roi_buf, MVE_COM_BUFFER_TYPE_ROI);
    ...
    {
        mve_buffer_set_owner4(buffer_client->buffer, MVE_BUFFER_OWNER_DEVICE, buffer_client->port_index);
        if (NULL != buffer_client->crc)
        {
            mve_buffer_set_owner4(buffer_client->crc, MVE_BUFFER_OWNER_DEVICE, buffer_client->port_index);
        }
    }

    if (false == empty_this_buffer)
    {
        ret = mve_com_add_output_buffer4(session, &buf, buffer_type);
    }
    else
    {
        ret = mve_com_add_input_buffer4(session, &buf, buffer_type);
    }
...
mve_base_error mve_session_buffer_dequeue_internal4(struct mve_session *session,
                                                   mve_base_buffer_handle_t buffer_id,
                                                   mve_com_buffer *buffer)
{
    ...
    buffer_client = mve_session_buffer_get_external_descriptor4(session, buffer_id);
    if (NULL == buffer_client)
    {
        /* The supplied user-space allocated buffer has not been registered
         * or is invalid. */
    ...
    if (false != buffer_info->do_cache_maintenance)
    {
         ...
         * call handles cache management */
        mve_buffer_set_owner4(buffer_client->buffer, MVE_BUFFER_OWNER_CPU, buffer_client->port_index);
        if (NULL != buffer_client->crc)
        {
            mve_buffer_set_owner4(buffer_client->crc, MVE_BUFFER_OWNER_CPU, buffer_client->port_index);
        }
    }
...
mve_base_error mve_session_buffer_notify_ref_frame_release_internal4(struct mve_session *session,
                                                                    mve_base_buffer_handle_t buffer_id)
{
    struct mve_buffer_client *buffer_client;
    ...
    return mve_com_add_message4(session, MVE_REQUEST_CODE_RELEASE_REF_FRAME, sizeof(data), (uint32_t *)&data);
}

void mve_session_buffer_convert_to_userspace4(struct mve_base_buffer_userspace *dst,
