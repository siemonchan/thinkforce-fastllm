...
#define MVE_RSRC_MEM_CACHE_ALLOC(size, flags) \
    mve_rsrc_mem_zalloc_track4(size, flags, ALLOCATOR_CACHE, __FUNCTION__, __LINE__)

#define MVE_RSRC_MEM_CACHE_FREE(ptr, size) \
    mve_rsrc_mem_free_track4(ptr, size, ALLOCATOR_CACHE, __FUNCTION__, __LINE__)

#else

#define MVE_RSRC_MEM_CACHE_ALLOC(size, flags) \
    mve_rsrc_mem_zalloc_track4(size, flags, ALLOCATOR_KMALLOC, __FUNCTION__, __LINE__)

#define MVE_RSRC_MEM_CACHE_FREE(ptr, size) \
    mve_rsrc_mem_free_track4(ptr, size, ALLOCATOR_KMALLOC, __FUNCTION__, __LINE__)
...
#define MVE_RSRC_MEM_FREE(ptr) \
    mve_rsrc_mem_free_track4(ptr, 0, ALLOCATOR_KMALLOC, __FUNCTION__, __LINE__)

#define MVE_RSRC_MEM_VALLOC(size) \
    mve_rsrc_mem_zalloc_track4(size, 0, ALLOCATOR_VMALLOC, __FUNCTION__, __LINE__)

#define MVE_RSRC_MEM_VFREE(ptr) \
    mve_rsrc_mem_free_track4(ptr, 0, ALLOCATOR_VMALLOC, __FUNCTION__, __LINE__)

#define MVE_RSRC_MEM_ALLOC_PAGE() \
    mve_rsrc_mem_alloc_page_track4(__FUNCTION__, __LINE__)

#define MVE_RSRC_MEM_FREE_PAGE(paddr) \
    mve_rsrc_mem_free_page_track4(paddr, __FUNCTION__, __LINE__)

#define MVE_RSRC_MEM_ALLOC_PAGES(nr_pages) \
    mve_rsrc_mem_alloc_pages_track4(nr_pages, __FUNCTION__, __LINE__)

#define MVE_RSRC_MEM_FREE_PAGES(paddrs, nr_pages) \
    mve_rsrc_mem_free_pages_track4(paddrs, nr_pages, __FUNCTION__, __LINE__)
 ...
 */
void *mve_rsrc_mem_zalloc_track4(uint32_t size,
                                uint32_t flags,
 ...
 */
void mve_rsrc_mem_free_track4(void *ptr,
                             uint32_t size,
                             enum mve_rsrc_mem_allocator allocator,
                             const char *func_str,
                             int line_nr);
 ...
 */
phys_addr_t mve_rsrc_mem_alloc_page_track4(const char *func_str, int line_nr);
 ...
 * @param line_nr  Line number of the free origin.
 */
void mve_rsrc_mem_free_page_track4(phys_addr_t paddr, const char *func_str, int line_nr);
 ...
 */
phys_addr_t *mve_rsrc_mem_alloc_pages_track4(uint32_t nr_pages,
                                            const char *func_str,
                                            int line_nr);
 ...
 * @param line_nr  Line number of the free origin.
 */
void mve_rsrc_mem_free_pages_track4(phys_addr_t *paddrs,
                                   uint32_t nr_pages,
                                   const char *func_str,
                                   int line_nr);
 ...
 */
ssize_t mve_rsrc_mem_print_stack4(char *buf);

/**
 * Clear the contents of the memory allocation table.
 */
void mve_rsrc_mem_clear_stack4(void);

/**
 * Enable/disable memory allocation failure simulation
 * @param enable True to enable, false to disable.
 */
void mve_rsrc_mem_resfail_enable4(bool enable);
...
#define MVE_RSRC_MEM_CACHE_ALLOC(size, flags) \
    mve_rsrc_mem_cache_alloc4(size, flags)

#define MVE_RSRC_MEM_CACHE_FREE(ptr, size) \
    mve_rsrc_mem_cache_free4(ptr, size)
#else
#define MVE_RSRC_MEM_CACHE_ALLOC(size, flags) \
    kzalloc(size, flags)
...
#define MVE_RSRC_MEM_ALLOC_PAGE() \
    mve_rsrc_mem_alloc_page4()

#define MVE_RSRC_MEM_FREE_PAGE(paddr) \
    mve_rsrc_mem_free_page4(paddr)

#define MVE_RSRC_MEM_ALLOC_PAGES(nr_pages) \
    mve_rsrc_mem_alloc_pages4(nr_pages)

#define MVE_RSRC_MEM_FREE_PAGES(paddrs, nr_pages) \
    mve_rsrc_mem_free_pages4(paddrs, nr_pages)
 ...
 */
void mve_rsrc_mem_init4(struct device *dev);

/**
 * Performs all necessary deinitialization of the memory frontend.
 */
void mve_rsrc_mem_deinit4(struct device *dev);
