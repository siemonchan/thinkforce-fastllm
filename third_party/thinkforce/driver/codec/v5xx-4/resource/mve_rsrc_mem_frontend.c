...
#ifdef EMULATOR
struct semaphore watchdog_sem4;
#endif /* EMULATOR */
                                ...
                                int line_nr)
{
    ...
    {
        ptr = mve_rsrc_mem_cache_alloc4(size, flags);
    }
    else
    {
...
phys_addr_t mve_rsrc_mem_alloc_page_track4(const char *func_str, int line_nr)
{
    ...
    paddr = mve_rsrc_mem_alloc_page4();
                                            ...
                                            int line_nr)
{
    ...
    paddrs = mve_rsrc_mem_alloc_pages4(nr_pages);
...
EXPORT_SYMBOL(mve_rsrc_mem_zalloc_track4);
EXPORT_SYMBOL(mve_rsrc_mem_free_track4);
EXPORT_SYMBOL(mve_rsrc_mem_alloc_page_track4);
EXPORT_SYMBOL(mve_rsrc_mem_free_page_track4);
EXPORT_SYMBOL(mve_rsrc_mem_alloc_pages_track4);
EXPORT_SYMBOL(mve_rsrc_mem_free_pages_track4);
...
static ssize_t sysfs_print_memory_table(struct device *dev,
                                        ...
                                        char *buf)
{
    return mve_rsrc_mem_print_stack4(buf);
}
#endif /* (1 == MVE_MEM_DBG_TRACKMEM) */
...
ssize_t sysfs_write_mem_resfail4(struct device *dev,
                                struct device_attribute *attr,
                                const char *buf,
                                size_t count)
{
    ...
    {
        mve_rsrc_mem_resfail_enable4(true);
    }
    else if (0 == strcmp("disable", buf))
    {
        mve_rsrc_mem_resfail_enable4(false);
    }
    else
    {
        int min, max;
        ...
        mve_rsrc_mem_resfail_set_range4(min, max);
    }
...
void mve_rsrc_mem_deinit4(struct device *dev)
{
    ...
    {
        char buf[4096] = "";
        mve_rsrc_mem_print_stack4(buf);
        printk(KERN_ERR "%s\n", buf);
        mve_rsrc_mem_clear_stack4();
    }
