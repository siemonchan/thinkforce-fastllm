...
void mver_pm_request_frequency3(uint32_t freq)
{
    ...
    dvfs_conf = (struct mve_dvfs_callback_conf *)mve_config_get_value3(mve_rsrc_data3.config,
                                                                      MVE_CONFIG_DEVICE_ATTR_DVFS_CALLBACKS);
    if (NULL == dvfs_conf || NULL == dvfs_conf->set_rate)
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "Unable to change the frequency.");
    }
    else
    {
...
uint32_t mver_pm_read_frequency3(void)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "Unable to read the frequency.");
        freq = 0;
    }
    else
    {
                                    ...
                                    size_t count)
{
    ...
    if (0 != freq)
    {
        mver_pm_request_frequency3(freq);
    }
...
void mver_pm_deinit3(struct device *dev)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "no dvfs_conf or stop.");
    }
    else
    {
        dvfs_conf->stop();
...
{
    mver_irq_disable3();
    return pm_runtime_put_sync(&mve_rsrc_data3.pdev->dev);
}
...
int mver_pm_request_resume3(void)
{
    ...
    ret = pm_runtime_get_sync(&mve_rsrc_data3.pdev->dev);
    val = mver_irq_enable3();
    if (0 != val)
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "Unable to register interrupt handler: \'%d\'", val);
    }

    return ret;
}

EXPORT_SYMBOL(mver_pm_request_frequency3);
EXPORT_SYMBOL(mver_pm_read_frequency3);
EXPORT_SYMBOL(mver_pm_request_suspend3);
EXPORT_SYMBOL(mver_pm_request_resume3);
...
int mver_pm_poweron3(void)
{
    ...
    pm_conf = (struct mve_pm_callback_conf *)mve_config_get_value3(mve_rsrc_data3.config,
...
{
    return mver_pm_suspend3(dev);
}

int mver_pm_runtime_resume3(struct device *dev)
{
    return mver_pm_resume3(dev);
}
