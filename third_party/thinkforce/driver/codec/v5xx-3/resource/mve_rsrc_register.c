...
{
    if (true == mve_rsrc_data3.hw_interaction)
    {
        writel(value, addr);
    }
}
...
tCS *mver_reg_get_coresched_bank3(void)
{
    ...
    return (tCS *)mve_rsrc_data3.regs;
}
...
{
    return mve_rsrc_data3.fuse;
}

uint32_t mver_reg_get_version3(void)
{
    return mve_rsrc_data3.hw_version;
}
...
{
    mve_rsrc_data3.hw_interaction = enable;
}
EXPORT_SYMBOL(mver_driver_set_hw_interaction3);

#endif

EXPORT_SYMBOL(mver_reg_write323);
EXPORT_SYMBOL(mver_reg_read323);
EXPORT_SYMBOL(mver_reg_get_coresched_bank3);
EXPORT_SYMBOL(mver_reg_put_coresched_bank3);
EXPORT_SYMBOL(mver_reg_get_fuse3);
EXPORT_SYMBOL(mver_reg_get_version3);
