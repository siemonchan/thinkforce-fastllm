...
#define MVE_DRIVER_NAME "mv500-3"

#define SET_BITFIELD(value, shift, sz) (((value) & ((1u << (sz)) - 1)) << (shift))

static bool probed = false;
static int mve_dev_major;
struct mve_rsrc_driver_data mve_rsrc_data3;

extern struct mve_config_attribute *mve_device_get_config3(void);
...
static void reset_hw(void)
{
    ...
    mver_reg_write323(&regs->RESET, 1);
    while (corelsid_mask !=  mver_reg_read323(&regs->CORELSID))
        ;
...
{
    struct mve_rsrc_driver_data *private = &mve_rsrc_data3;
    ...
    private->config = mve_device_get_config3();
    ...
    attr_fptr = (mve_port_attributes_callback_fptr)
                mve_config_get_value3(private->config, MVE_CONFIG_DEVICE_ATTR_BUS_ATTRIBUTES);
    if (NULL == attr_fptr)
    {
    ...
    mver_reg_init3();
    mve_rsrc_mem_init3(&pdev->dev);
    mve_rsrc_log_init3();
    mver_irq_handler_init3(&pdev->dev);
    mver_pm_init3(&pdev->dev);
    mver_scheduler_init3(&pdev->dev);
    ...
    reset_hw();
    regs = mver_reg_get_coresched_bank3();
    private->nlsid = mver_reg_read323(&regs->NLSID);
    private->ncore = mver_reg_read323(&regs->NCORES);
    private->fuse = mver_reg_read323(&regs->FUSE);
    private->hw_version = mver_reg_read323(&regs->VERSION);
    ...
    if (private->hw_version >= MVE_V52_V76)
    {
        ...
        mver_reg_write323(&regs->BUSCTRL, sysbusctrl);
    }
...
{
    mver_scheduler_deinit3(&pdev->dev);
    mver_irq_handler_deinit3(&pdev->dev);
    mve_rsrc_log_destroy3();
    mve_rsrc_mem_deinit3(&pdev->dev);
    mver_pm_deinit3(&pdev->dev);
    pm_runtime_disable(&pdev->dev);

    iounmap(mve_rsrc_data3.regs);
    release_mem_region(mve_rsrc_data3.mem_res->start, resource_size(mve_rsrc_data3.mem_res));
    unregister_chrdev(mve_dev_major, MVE_RSRC_DRIVER_NAME);
...
{
    .suspend = mver_pm_suspend3,
    .resume = mver_pm_resume3,
    .runtime_suspend = mver_pm_runtime_suspend3,
    .runtime_resume = mver_pm_runtime_resume3,
    .runtime_idle = mver_pm_runtime_idle3,
};
#endif
...
static long mve_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    switch (cmd)
    {
        case MVE_BASE_COMMAND:
        {
            ...
            {
                MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "copy_from_user() failed to get the MVE_COMMAND header.");
                return -EFAULT;
            }
            ...
            if (0 < header.size)
            {
                ...
                if (NULL == data)
                {
                    MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "Failed to allocate memory.");
                    return -ENOMEM;
                }
                ...
                if (0 != copy_from_user(data,
                                        ...
                                        header.size))
                {
                    MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "copy_from_user() failed to get MVE_COMMAND data.");
                    MVE_RSRC_MEM_CACHE_FREE(data, header.size);
                    return -EFAULT;
                }
            }
            /* Process command */
            result = mve_command_execute3(&header, data, file);
            ...
            {
                MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "copy_to_user() failed to copy MVE_COMMAND header.");
                return -EFAULT;
            }
            ...
            if (0 < result->size)
            {
                ...
                {
                    MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "copy_to_user() failed to copy MVE_COMMAND data.");
                    return -EFAULT;
                }
        ...
        default:
            MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "Unknown ioctl. cmd=%u.", cmd);
            break;
    }
    return 0;
}
...
static int mve_driver_release(struct inode *inode, struct file *file)
{
    ...
    mve_session_cleanup_client3(file);
...
{
    return mve_session_poll3(filp, poll_table);
}
...
static int mve_driver_probe(struct platform_device *pdev)
{
    ...
    mve_device3 = &pdev->dev;

    /* Allocate MVE device. */
    mdev = MVE_RSRC_MEM_ZALLOC(sizeof(*mve_device3), GFP_KERNEL);
...
static void __exit mver_driver_exit(void)
{
    ...
    MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_INFO, "MVE resource driver unregistered.");
}
