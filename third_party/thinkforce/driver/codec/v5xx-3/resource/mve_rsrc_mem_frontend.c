...
#ifdef EMULATOR
struct semaphore watchdog_sem3;
#endif /* EMULATOR */
                                ...
                                int line_nr)
{
    ...
    {
        ptr = mve_rsrc_mem_cache_alloc3(size, flags);
    }
    else
    {
...
phys_addr_t mve_rsrc_mem_alloc_page_track3(const char *func_str, int line_nr)
{
    ...
    paddr = mve_rsrc_mem_alloc_page3();
                                            ...
                                            int line_nr)
{
    ...
    paddrs = mve_rsrc_mem_alloc_pages3(nr_pages);
...
EXPORT_SYMBOL(mve_rsrc_mem_zalloc_track3);
EXPORT_SYMBOL(mve_rsrc_mem_free_track3);
EXPORT_SYMBOL(mve_rsrc_mem_alloc_page_track3);
EXPORT_SYMBOL(mve_rsrc_mem_free_page_track3);
EXPORT_SYMBOL(mve_rsrc_mem_alloc_pages_track3);
EXPORT_SYMBOL(mve_rsrc_mem_free_pages_track3);
...
static ssize_t sysfs_print_memory_table(struct device *dev,
                                        ...
                                        char *buf)
{
    return mve_rsrc_mem_print_stack3(buf);
}
#endif /* (1 == MVE_MEM_DBG_TRACKMEM) */
...
ssize_t sysfs_write_mem_resfail3(struct device *dev,
                                struct device_attribute *attr,
                                const char *buf,
                                size_t count)
{
    ...
    {
        mve_rsrc_mem_resfail_enable3(true);
    }
    else if (0 == strcmp("disable", buf))
    {
        mve_rsrc_mem_resfail_enable3(false);
    }
    else
    {
        int min, max;
        ...
        mve_rsrc_mem_resfail_set_range3(min, max);
    }
...
void mve_rsrc_mem_deinit3(struct device *dev)
{
    ...
    {
        char buf[4096] = "";
        mve_rsrc_mem_print_stack3(buf);
        printk(KERN_ERR "%s\n", buf);
        mve_rsrc_mem_clear_stack3();
    }
