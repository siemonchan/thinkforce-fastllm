                                                        ...
                                                        struct file *filep)
{
    ...
    if (false == is_role_enabled(role))
    {
        ...
        MVE_LOG_PRINT(&mve_rsrc_log_session3,
                      MVE_LOG_ERROR,
    ...
    {
        session = mve_session_create3(filep);
        if (NULL == session)
        {
        ...
        {
            mve_session_set_role3(session, role);
        }
    }
...
struct mve_response *mve_command_execute3(struct mve_base_command_header *header,
                                         void *data,
                                         struct file *filep)
{
#define MVE_LOG_COMMAND(_severity)                                                                                   \
    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3,                                                                     \
                          _severity,                                                                                 \
                          session,                                                                                   \
                          "mve_command_execute3(header={cmd=%u \"%s\", size=%u, data[0]=%u}, data=0x%x, file=0x%x).", \
                          header->cmd,                                                                               \
    ...
    if (NULL == ret)
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "Failed to allocate memory for the ioctl return struct.");
        return NULL;
    }
...
//    printk("%s : cmd %d %s enter\n", __func__, header->cmd, mve_command_to_string(header->cmd));
    ...
    switch (header->cmd)
    {
        ...
        case MVE_BASE_ACTIVATE_SESSION:
        {
            ...
            if (NULL != version)
            {
                res = mve_session_activate3(session, version, fw_secure_desc);
                if (false != res)
                {
                    ret->data = version;
        ...
        case MVE_BASE_ENQUEUE_FLUSH_BUFFERS:
        {
            ...
            ret->error = mve_session_enqueue_flush_buffers3(session, flush);
            break;
        }
        case MVE_BASE_ENQUEUE_STATE_CHANGE:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_enqueue_state_change3(session, state);
            break;
        }
        case MVE_BASE_GET_EVENT:
        {
            ...
            event = mve_session_get_event3(session, timeout);
            if (NULL == event)
            {
        ...
        {
            ret->error = mve_session_set_paramconfig3(session,
                                                     header->size,
                                                     data,
                                                     &ret->firmware_error,
                                                     header->cmd);
        ...
        case MVE_BASE_REGISTER_BUFFER:
        {
            ...
            {
                ret->error = mve_session_buffer_register3(session, port_index, descriptor);
            }
            break;
        }
        case MVE_BASE_UNREGISTER_BUFFER:
        {
            ...
            {
                MVE_LOG_COMMAND(MVE_LOG_INFO);
                ret->error = mve_session_buffer_unregister3(session, buffer_id);
            }
            break;
        }
        case MVE_BASE_FILL_THIS_BUFFER:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_buffer_enqueue3(session, param, false);
            break;
        }
        case MVE_BASE_EMPTY_THIS_BUFFER:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_buffer_enqueue3(session, param, true);
            break;
        }
        case MVE_BASE_NOTIFY_REF_FRAME_RELEASE:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_buffer_notify_ref_frame_release3(session, buffer_id);
            break;
        }
        ...
        case MVE_BASE_READ_HW_INFO:
        {
            ...
            else
            {
                ...
                hw_info->fuse = mver_reg_get_fuse3();
                hw_info->version = mver_reg_get_version3();
                hw_info->ncores = mver_scheduler_get_ncores3();
            }
            break;
        }
        case MVE_BASE_RPC_MEM_ALLOC:
        {
            ...
            fd = *((int32_t *)data);
            ret->error = mve_session_handle_rpc_mem_alloc3(session, fd);
#endif      /* EMULATOR */
            break;
        }
        case MVE_BASE_RPC_MEM_RESIZE:
        {
            ...
            fd = *((int32_t *)data);
            ret->error = mve_session_handle_rpc_mem_resize3(session, fd);
#endif      /* EMULATOR */
            break;
        }
#ifdef UNIT
        case MVE_BASE_DEBUG_READ_REGISTER:
            ...
            else
            {
                ...
                value = mver_reg_read323(base + offset);
        ...
        case MVE_BASE_DEBUG_WRITE_REGISTER:
            ...
            WARN_ON(header->size != sizeof(uint32_t) * 2);
            {
                ...
                mver_reg_write323(base + offset, value);
                mver_reg_put_coresched_bank3(&regs);
            }
            break;
        case MVE_BASE_DEBUG_INTERRUPT_COUNT:
            ...
            {
                *((uint32_t *)ret->data) = mve_rsrc_data3.interrupts;
                ret->size = sizeof(uint32_t);
            }
            break;
        case MVE_BASE_DEBUG_SEND_COMMAND:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_enqueue_message3(session, ptr[0], ptr[1], &ptr[2]);
            break;
        }
        case MVE_BASE_DEBUG_FIRMWARE_HUNG_SIMULATION:
        {
            ...
            mve_session_firmware_hung_simulation3(session, on);
#endif
