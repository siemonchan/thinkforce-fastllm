...
static void free_session(struct kref *ref)
{
    ...
    mve_session_buffer_unmap_all3(session);

    mve_queue_destroy3(session->response_events);
    mve_queue_destroy3(session->mve_events);

    mve_mem_virt_region_get3(VIRT_MEM_REGION_MSG_IN_QUEUE, &region);
    mve_mmu_unmap_pages3(session->mmu_ctx, region.start);

    mve_mem_virt_region_get3(VIRT_MEM_REGION_MSG_OUT_QUEUE, &region);
    mve_mmu_unmap_pages3(session->mmu_ctx, region.start);

    mve_mem_virt_region_get3(VIRT_MEM_REGION_INPUT_BUFFER_IN, &region);
    mve_mmu_unmap_pages3(session->mmu_ctx, region.start);

    mve_mem_virt_region_get3(VIRT_MEM_REGION_INPUT_BUFFER_OUT, &region);
    mve_mmu_unmap_pages3(session->mmu_ctx, region.start);

    mve_mem_virt_region_get3(VIRT_MEM_REGION_OUTPUT_BUFFER_IN, &region);
    mve_mmu_unmap_pages3(session->mmu_ctx, region.start);

    mve_mem_virt_region_get3(VIRT_MEM_REGION_OUTPUT_BUFFER_OUT, &region);
    mve_mmu_unmap_pages3(session->mmu_ctx, region.start);

    mve_mem_virt_region_get3(VIRT_MEM_REGION_RPC_QUEUE, &region);
    mve_mmu_unmap_pages3(session->mmu_ctx, region.start);

    mve_rsrc_dma_mem_free3(session->msg_in_queue);
    mve_rsrc_dma_mem_free3(session->msg_out_queue);
    mve_rsrc_dma_mem_free3(session->buf_input_in);
    mve_rsrc_dma_mem_free3(session->buf_input_out);
    mve_rsrc_dma_mem_free3(session->buf_output_in);
    mve_rsrc_dma_mem_free3(session->buf_output_out);
    mve_rsrc_dma_mem_free3(session->rpc_area);

    MVE_RSRC_MEM_FREE(session->role);
    ret = mve_fw_unload3(session->mmu_ctx, session->fw_inst);
    if (false == ret)
    {
        MVE_LOG_PRINT(&mve_rsrc_log_session3, MVE_LOG_DEBUG, "Firmware unload failed.");
    }
    mve_mmu_destroy_ctx3(session->mmu_ctx);
...
static bool change_session_state(struct mve_session *session, enum MVE_SESSION_STATE to_state)
{
    ...
    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_DEBUG, session, "Changing state. '%s' -> '%s'.", state_to_char[session->state.state], state_to_char[to_state]);
    ...
    {
        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_ERROR, session, "Invalid state change. '%s' -> '%s'.", state_to_char[session->state.state], state_to_char[to_state]);
    }
                                      ...
                                      void *data)
{
    ...
    ret = mve_com_add_message3(session, code, size, data);
...
{
    mve_queue_add_event3(session->mve_events, event);
#ifdef EMULATOR
    {
...
static void return_pending_buffer_to_userspace(struct mve_session *session, struct session_enqueue_request *request, bool empty_this_buffer)
{
    ...
    event = mve_queue_create_event3(event_code, sizeof(struct mve_base_buffer_userspace));
    if (NULL == event)
    {
        return;
    }
...
static mve_base_error enqueue_job(struct mve_session *session)
{
    ...
    cmd.cores = mver_scheduler_get_ncores3();
...
static void enqueue_pending_buffers(struct mve_session *session, struct list_head *list, bool empty_this_buffer)
{
    ...
    list_for_each_safe(pos, next, list)
    {
        ...
        ret = mve_session_buffer_enqueue_internal3(session, &request->param, empty_this_buffer);
...
static bool schedule_session(struct mve_session *session)
{
    ...
    {
        ret = mver_scheduler_execute3((mver_session_id)session);
    }
...
static void switchout_session(struct mve_session *session)
{
        ...
        STATE_SWITCHING_IN != session->state.state)
    {
        ...
        if (false != ret)
        {
            ...
            mver_irq_signal_mve3((mver_session_id)session);
        }
    }
...
{
    struct mve_base_fw_version *ver = mve_fw_get_version3(session->fw_inst);
    bool ret = false;
...
static void process_generic_event(struct mve_session *session, struct mve_base_event_header *event)
{
    ...
    switch (event->code)
    {
        ...
        case MVE_BASE_EVENT_INPUT:
        {
            ...
            {
                mve_com_get_output_buffer3(session, &buffer);
            }
            else
            {
                mve_com_get_input_buffer3(session, &buffer);
            }
            ...
            if (NULL != buffer_client)
            {
                ...
                else
                {
                    ...
                    res = mve_session_buffer_dequeue_internal3(session, buffer_client->buffer->info.buffer_id, &buffer);
                        ...
                        MVE_BASE_BUFFERFLAG_EOS == (buffer_client->flags & MVE_BASE_BUFFERFLAG_EOS))
                    {
                        ...
                        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_DEBUG, session, "Resetting eos_queued.");
                    }

                    usr_event = mve_queue_create_event3(event->code, sizeof(struct mve_base_buffer_userspace));
                    if (NULL == usr_event)
                    {
                        return;
                    }
                    ...
                    mve_session_buffer_convert_to_userspace3(buf, buffer_client);
            ...
            if (0 != (session->state.flush_target & MVE_BASE_FLUSH_QUICK))
            {
                    ...
                    session->state.flush_state)
                {
                    ...
                    list_for_each_safe(pos, next, &session->quick_flush_buffers)
                    {
                        ...
                        param.timestamp = 0;
                        ret = mve_session_buffer_enqueue_internal3(session,
                                                                  &param,
        ...
        case MVE_BASE_EVENT_FW_ERROR:
        {
            ...
            {
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_ERROR, session, "Unsupported firmware.");
                ...
                if (STATE_WAITING_FOR_RESPONSE == session->state.state)
                {
                    mve_queue_interrupt3(session->response_events);
                }
            }
            else if (NULL != e && MVE_ERROR_WATCHDOG == e->error_code && !strcmp(e->message, "Watchdog"))
            {
                ...
                host_area = mve_rsrc_dma_mem_map3(session->msg_in_queue);
                mve_area = mve_rsrc_dma_mem_map3(session->msg_out_queue);

                if (host_area != NULL && mve_area != NULL)
                {
                    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_ERROR, session, "host_area wpos: %d mve_area rpos: %d.", host_area->in_wpos, mve_area->in_rpos);
                }
                mve_rsrc_dma_mem_unmap3(session->msg_in_queue);
                mve_rsrc_dma_mem_unmap3(session->msg_out_queue);
            }
            break;
        }
        ...
        case MVE_BASE_EVENT_REF_FRAME_RELEASED:
        {
            ...
            if (NULL != buffer_client)
            {
                ...
                struct mve_base_event_header *usr_event;
                usr_event = mve_queue_create_event3(event->code, sizeof(mve_base_buffer_handle_t));
                if (NULL == usr_event)
                {
                    return;
                }
        ...
        default:
        {
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_ERROR, session, "Illegal event code. code=%u.", event->code);
            return;
        }
    }
...
static void handle_mve_message(struct mve_session *session)
{
    ...
    data = mve_com_get_message3(session, &header);
    while (NULL != data)
    {
        enum mve_base_event_code event_code;
        struct mve_base_event_header *event;
        ...
        event = mve_queue_create_event3(event_code, size);
        if (NULL != event)
        {
            ...
            if (MVE_BASE_EVENT_FW_ERROR == event->code)
            {
                ...
                MVE_LOG_PRINT(&mve_rsrc_log_session3, MVE_LOG_ERROR, "Firmware error. reason=%u, msg=%s.", err->reason, err->msg);
            }
            ...
            switch (event_code)
            {
                case MVE_BASE_EVENT_SET_PARAMCONFIG:
                    ...
                    response_received(session);
                    mve_queue_add_event3(session->response_events, event);
#ifdef EMULATOR
                    {
                        int t = (int)(((uintptr_t)session->filep) & 0xFFFFFFFF);
                        write(t, &t, sizeof(int));
...
mve_base_error mve_session_handle_rpc_mem_alloc3(struct mve_session *session, int fd)
{
    ...
    err = mve_com_get_rpc_message3(session, &rpc);
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        buffer = mve_buffer_create_buffer_external3(&info, 0);
        if (NULL != buffer)
        {
            bool ret = mve_buffer_map_physical_pages3(buffer);
        ...
        if (NULL != pages)
        {
            ...
            {
                mve_mem_virt_region_get3(VIRT_MEM_REGION_PROTECTED, &region);
            }
            else
            {
                mve_mem_virt_region_get3(VIRT_MEM_REGION_OUT_BUF, &region);
            }

            ret = mve_mmu_map_pages_and_reserve3(session->mmu_ctx,
                                                pages,
                                                &region,
                                                num_pages,
                                                max_pages,
            ...
            mve_buffer_unmap_physical_pages3(buffer);
            mve_buffer_destroy_buffer_external3(buffer);
            buffer = NULL;
        }

        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_DEBUG, session,
...
error:
    ...
    mve_com_put_rpc_message3(session, &rpc);
...
mve_base_error mve_session_handle_rpc_mem_resize3(struct mve_session *session, int fd)
{
    ...
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        ret = mve_mmu_map_info3(session->mmu_ctx,
                               mve_addr,
                               &mapped_pages,
                               &max_pages);
        ...
        if (NULL != buffer)
        {
            ret = mve_buffer_map_physical_pages3(buffer);
            if (false != ret && num_new_pages <= buffer->mapping.num_pages)
            {
                ret = mve_mmu_map_resize3(session->mmu_ctx,
...
error:
        mver_scheduler_flush_tlb3((mver_session_id)session);

        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_DEBUG, session, "RPC resize requested. new_size=%d, fd: %d, num_pages=%d, addr=0x%X ret_addr= %#x.", new_size, fd, num_pages, mve_addr, ret_addr);
...
static void handle_rpc_request(struct mve_session *session)
{
    ...
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        {
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_INFO, session, "<- RPC %s", print_rpc_code(rpc.call_id));
        }
        ...
        switch (rpc.call_id)
        {
            case MVE_COM_RPC_FUNCTION_DEBUG_PRINTF:
            {
                ...
                event = mve_queue_create_event3(MVE_BASE_EVENT_RPC_PRINT, size);
                if (NULL == event)
                {
                    return;
                }
            ...
            case MVE_COM_RPC_FUNCTION_MEM_ALLOC:
            {
                ...
                phys_addr_t *pages;
                uint32_t mve_addr = 0;
                bool secure = mve_fw_secure3(session->fw_inst);
...
//                num_sys_pages = (size + PAGE_SIZE - 1) / PAGE_SIZE;
                ...
                if (false != secure)
                {
                    ...
                    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_DEBUG, session, "%s send event to userspace size: %d", __FUNCTION__, num_pages * MVE_MMU_PAGE_SIZE);

                    event = mve_queue_create_event3(MVE_BASE_EVENT_RPC_MEM_ALLOC, sizeof(struct mve_base_rpc_memory));
                    if (NULL != event)
                    {
                ...
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_INFO, session,
            ...
            case MVE_COM_RPC_FUNCTION_MEM_RESIZE:
            {
                ...
                if (false != secure)
                {
                    ...
                    bool ret = mve_mmu_map_info3(session->mmu_ctx,
                                                mve_addr,
                                                &mapped_pages,
                                                &max_pages);
                    if (false == ret || num_pages > max_pages)
                    ...
                    if (num_pages > mapped_pages)
                    {
                        ...
                        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_DEBUG, session, "%s send event to userspace old: %d extra: %d",
                                              __FUNCTION__, mapped_pages * MVE_MMU_PAGE_SIZE, num_new_pages * MVE_MMU_PAGE_SIZE);

                        event = mve_queue_create_event3(MVE_BASE_EVENT_RPC_MEM_RESIZE, sizeof(struct mve_base_rpc_memory));
                        if (NULL != event)
                        {
                            rpc_memory = (struct mve_base_rpc_memory *)event->data;
                            rpc_memory->region = VIRT_MEM_REGION_PROTECTED == mve_mem_virt_region_type_get3(mve_addr) ?
                ...
                {
                    bool ret = mve_mmu_map_resize3(session->mmu_ctx,
                                                  NULL,
                                                  mve_addr,
                                                  num_pages,
                                                  ATTRIB_SHARED_RW,
...
resize_error:
                ...
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_INFO, session, "RPC resize requested. new_size=%d, num_pages=%d, mve_addr=0x%X, ret_addr: 0x%X.", new_size, num_pages, mve_addr, ret_addr);
            ...
            case MVE_RPC_FUNCTION_MEM_FREE:
            {
                ...
                {
                    mve_mmu_unmap_pages3(session->mmu_ctx, rpc.params.mem_free.ve_pointer);
                }

                mver_scheduler_flush_tlb3((mver_session_id)session);

                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_INFO, session, "RPC free requested. addr=0x%X.", rpc.params.mem_free.ve_pointer);
            ...
            default:
            {
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_ERROR, session, "Unsupported RPC request. id=%u.", rpc.call_id);
                rpc.size = 0;
            }
        }
 ...
 */
void mve_session_irq_callback3(mver_session_id session_id)
{
...
static void mve_session_hung(struct mve_session *session)
{
    ...
    event = mve_queue_create_event3(MVE_BASE_EVENT_SESSION_HUNG, 0);
    if (NULL == event)
    {
        return;
    }
    ...
    mver_scheduler_stop3((mver_session_id)session);
    mver_scheduler_unregister_session3((mver_session_id)session);
}
...
static void session_destroy_instance(struct mve_session *session)
{
    ...
    mve_com_delete3(session);

    mver_pm_request_suspend3();
    ...
    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_INFO, session, "Session destroyed.");

    mver_scheduler_lock3();
    sem_taken = down_interruptible(&sessions_sem);
...
static void mve_session_switchin_callback(mver_session_id session_id)
{
    ...
    else
    {
        ...
        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3,
                              MVE_LOG_ERROR,
                              session,
    ...
    mver_irq_signal_mve3((mver_session_id)session_id);
...
static void mve_session_switchout_completed_callback(mver_session_id session_id)
{
    ...
    mve_session_irq_callback3(session_id);
...
#define TRACE_SIZE 128 * 1024
    ...
    if (NULL == str)
    {
        MVE_LOG_PRINT(&mve_rsrc_log_session3, MVE_LOG_ERROR, "Unable to allocate memory for FW trace buffer.");
        return -EBUSY;
    }
                ...
                if ((1 << rasc) & fw_trace->rasc_mask)
                {
                    ...
                    if (0 != rasc_addr)
                    {
                        ...
                        if (NULL != buffer)
                        {
                            ...
                            mve_mmu_read_buffer3(session->mmu_ctx, rasc_addr, (uint8_t *)buffer, size);
...
void mve_session_init3(struct device *dev)
{
...
#if defined(_DEBUG) && !defined(EMULATOR)
    {
        ...
        {
            MVE_LOG_PRINT(&mve_rsrc_log_session3, MVE_LOG_ERROR, "Unable to create debugfs file 'fw_trace'.");
        }
    }
...
#if defined(CONFIG_SYSFS)
    {
        ...
        for (i = 0; i < NELEMS(sysfs_files); ++i)
        {
            ...
            if (err < 0)
            {
                MVE_LOG_PRINT(&mve_rsrc_log_session3, MVE_LOG_ERROR, "Unable to create sysfs file.");
            }
        }
    }
#endif /* CONFIG_SYSFS */
...
void mve_session_deinit3(struct device *dev)
{
    ...
    mve_session_destroy_all_sessions3();
}
...
struct mve_session *mve_session_create3(struct file *filep)
{
    ...
    session->mmu_ctx = mve_mmu_create_ctx3();
    ...
    session->msg_in_queue   = mve_rsrc_dma_mem_alloc3(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->msg_out_queue  = mve_rsrc_dma_mem_alloc3(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_input_in   = mve_rsrc_dma_mem_alloc3(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_input_out  = mve_rsrc_dma_mem_alloc3(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_output_in  = mve_rsrc_dma_mem_alloc3(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_output_out = mve_rsrc_dma_mem_alloc3(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->rpc_area       = mve_rsrc_dma_mem_alloc3(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    ...
    mve_mem_virt_region_get3(VIRT_MEM_REGION_MSG_IN_QUEUE, &region);
    pages = mve_rsrc_dma_mem_get_pages3(session->msg_in_queue);
    ret = mve_mmu_map_pages3(session->mmu_ctx, pages,
    ...
    mve_mem_virt_region_get3(VIRT_MEM_REGION_MSG_OUT_QUEUE, &region);
    pages = mve_rsrc_dma_mem_get_pages3(session->msg_out_queue);
    ret = mve_mmu_map_pages3(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_mem_virt_region_get3(VIRT_MEM_REGION_INPUT_BUFFER_IN, &region);
    pages = mve_rsrc_dma_mem_get_pages3(session->buf_input_in);
    ret = mve_mmu_map_pages3(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_mem_virt_region_get3(VIRT_MEM_REGION_INPUT_BUFFER_OUT, &region);
    pages = mve_rsrc_dma_mem_get_pages3(session->buf_input_out);
    ret = mve_mmu_map_pages3(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_mem_virt_region_get3(VIRT_MEM_REGION_OUTPUT_BUFFER_IN, &region);
    pages = mve_rsrc_dma_mem_get_pages3(session->buf_output_in);
    ret = mve_mmu_map_pages3(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_mem_virt_region_get3(VIRT_MEM_REGION_OUTPUT_BUFFER_OUT, &region);
    pages = mve_rsrc_dma_mem_get_pages3(session->buf_output_out);
    ret = mve_mmu_map_pages3(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    /* Map RPC area */
    mve_mem_virt_region_get3(VIRT_MEM_REGION_RPC_QUEUE, &region);
    pages = mve_rsrc_dma_mem_get_pages3(session->rpc_area);
    ret = mve_mmu_map_pages3(session->mmu_ctx, pages,
                            region.start, 1, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    session->mve_events = mve_queue_create3(16384);
    session->response_events = mve_queue_create3(8192);
    ...
    mver_pm_request_resume3();

#ifndef DISABLE_WATCHDOG
    watchdog_create_session();
#endif

    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_INFO, session, "Session created.");
...
void mve_session_cleanup_client3(struct file *filep)
{
    ...
    session = mve_session_get_by_file3(filep);
    if (NULL != session)
    {
        int sem_taken = down_interruptible(&sessions_sem);
        list_del(&session->list);
                          ...
                          struct mve_base_fw_secure_descriptor *fw_secure_desc)
{
    ...
    ncores = mver_scheduler_get_ncores3();
    ...
    session->fw_inst = mve_fw_load3(session->mmu_ctx, fw_secure_desc, session->role, ncores);
    if (NULL != session->fw_inst)
    {
        ...
        ver = mve_fw_get_version3(session->fw_inst);
        if (NULL != ver)
        {
        ...
        error = mve_com_set_interface_version3(session, session->prot_ver);
        mve_fw_log_fw_binary3(session->fw_inst, session);

        if (error == MVE_BASE_ERROR_NONE)
        {
            l1_page = mve_mmu_get_id3(session->mmu_ctx);
            mver_scheduler_register_session3((mver_session_id)session,
                                            mve_mmu_make_l0_entry(ATTRIB_PRIVATE, l1_page),
                                            ncores,
                                            mve_fw_secure3(session->fw_inst),
                                            mve_session_irq_callback3,
...
mve_base_error mve_session_enqueue_flush_buffers3(struct mve_session *session, uint32_t flush)
{
    bool res;
...
mve_base_error mve_session_enqueue_state_change3(struct mve_session *session, enum mve_base_hw_state state)
{
    bool res;
    mve_base_error ret = MVE_BASE_ERROR_NONE;
...
struct mve_base_event_header *mve_session_get_event3(struct mve_session *session, uint32_t timeout)
{
    ...
    if (true == session->fw_loaded)
    {
        event = mve_queue_wait_for_event3(session->mve_events, timeout);
    }
                                           ...
                                           enum mve_base_command_type entity)
{
    ...
    if (MVE_BASE_ERROR_NONE == ret)
    {
        ...
        {
            unlock_session(session);
            event = mve_queue_wait_for_event3(session->response_events, 10000);
            lock_session(session);

            if (NULL == event)
            {
                if (false != mve_queue_interrupted3(session->response_events))
                {
                    ret = MVE_BASE_ERROR_HARDWARE;
                }
                else
                                           ...
                                           struct mve_base_buffer_userspace *descriptor)
{
    ...
    {
        uint32_t fuse = mver_reg_get_fuse3();
    ...
    mver_scheduler_lock3();
    lock_session(session);
    ret = mve_session_buffer_register_internal3(session, port_index, descriptor);
    unlock_session(session);
    mver_scheduler_unlock3();
    release_session(session);
                                             ...
                                             mve_base_buffer_handle_t buffer_id)
{
    ...
    mver_scheduler_lock3();
    lock_session(session);
    ret = mve_session_buffer_unregister_internal3(session, buffer_id);
    unlock_session(session);
    mver_scheduler_unlock3();
    release_session(session);
                                          ...
                                          bool empty_this_buffer)
{
    ...
    if (false != empty_this_buffer)
    {
        ...
        {
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session3, MVE_LOG_DEBUG, session, "Setting eos_queued.");
        }
    }
    else
    {
        ...
        (false == empty_this_buffer && (0 != (session->state.flush_target & MVE_BASE_FLUSH_OUTPUT_PORT))))
    {
        if (session->state.flush_target & MVE_BASE_FLUSH_QUICK)
        {
            struct mve_buffer_client *buffer_client = mve_session_buffer_get_external_descriptor3(session, param->buffer_id);
    ...
    ret = mve_session_buffer_enqueue_internal3(session, param, empty_this_buffer);
                                                           ...
                                                           mve_base_buffer_handle_t buffer_id)
{
    ...
    lock_session(session);
    mve_session_buffer_notify_ref_frame_release_internal3(session, buffer_id);
    unlock_session(session);
    res = schedule_session(session);
    if (false != res)
    {
...
uint32_t mve_session_poll3(struct file *filp, struct poll_table_struct *poll_table)
{
    ...
    session = mve_session_get_by_file3(filp);
    if (NULL != session)
    {
        lock_session(session);
        ...
        if (false != mve_queue_event_ready3(session->mve_events))
        {
