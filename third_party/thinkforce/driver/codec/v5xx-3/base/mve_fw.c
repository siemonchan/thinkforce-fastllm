...
static struct mve_firmware_descriptor *load_firmware(const char *filename)
{
    ...
    ret = request_firmware(&fw, filename, mve_device3);
    if (0 > ret)
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "Unable to load firmware file. file=%s.", filename);
        goto error;
    }
    ...
    while (0 < bytes_left)
    {
        ...
        void *dst = mve_rsrc_mem_cpu_map_page3(desc->data[i]);
        ...
        mve_rsrc_mem_cpu_unmap_page3(desc->data[i]);
        mve_rsrc_mem_clean_cache_range3(desc->data[i], PAGE_SIZE);
        i++;
        bytes_left -= size;
    }
...
static bool construct_fw_page_table(struct mve_firmware_descriptor *desc)
{
    ...
    header = (struct fw_header *)mve_rsrc_mem_cpu_map_page3(desc->data[0]);
    if (NULL == header)
    {
        return false;
    }
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_INFO, "Detected firmware version v2. major=%u, minor=%u.", desc->fw_version.major, desc->fw_version.minor);
    }
    else if (MVE_BASE_FW_MAJOR_PROT_V3 == desc->fw_version.major)
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_INFO, "Detected firmware version v3. major=%u, minor=%u.", desc->fw_version.major, desc->fw_version.minor);
    }
    else
    {
        MVE_LOG_PRINT(&mve_rsrc_log3, MVE_LOG_ERROR, "Unknown firmware version. major=%u, minor=%u.", desc->fw_version.major, desc->fw_version.minor);
        desc->fw_version.major = MVE_BASE_FW_MAJOR_PROT_UNKNOWN;
    }
    ret = true;

out:
    mve_rsrc_mem_cpu_unmap_page3(desc->data[0]);
                         ...
                         struct mve_fw_instance *inst)
{
    ...
    if (true == first_core || FW_PAGETYPE_BSS == page_type)
    {
        ...
        for (j = first_bss_idx; j <= last_bss_idx; j++)
        {
            ...
            mve_rsrc_mem_write323(dst_page + MVE_MMU_PAGE_TABLE_ENTRY_SIZE * j, entry);
        }
    }
                                                    ...
                                                    int ncores)
{
    ...
    for (i = 0; i < (unsigned int)ncores; ++i)
    {
        ...
        mve_mem_virt_region_get3(fw_regions[i], &region);
        ...
        if (true == first_core)
        {
            ...
            for (j = 0; j < (MVE_MMU_PAGE_SIZE / sizeof(mve_mmu_entry_t)); j++)
            {
                ...
                if (FW_GET_TYPE(fw_mmu_page_line) == FW_PAGETYPE_TEXT)
                {
                    ...
                    mve_rsrc_mem_write323(l2page + 4 * j, entry);
                }
            }
        }
        ...
        ret = mve_mmu_map_pages_merge3(ctx,
                                      l2page,
                                      region.start,
                                      MVE_MMU_PAGE_SIZE / sizeof(uint32_t));
        if (false == ret)
...
error:
    ...
    if (NULL != desc)
    {
        ...
        for (i = 0; i < ncores_setup; ++i)
        {
            ...
            mve_mem_virt_region_get3(fw_regions[i], &region);
            mve_mmu_unmap_pages3(ctx, region.start);
        }
                                                  ...
                                                  int ncores)
{
    ...
    for (i = 0; i < (unsigned int)ncores; ++i)
    {
        ...
        ret = mve_mmu_map_page_replace3(ctx,
                                       l2page,
                                       region.start);
        if (false == ret)
        {
...
void mve_fw_log_fw_binary3(struct mve_fw_instance *inst, struct mve_session *session)
{
    ...
    header = (struct fw_header *)mve_rsrc_mem_cpu_map_page3(inst->desc->data[0]);
    if (NULL == header)
    {
        return;
    }
    ...
    MVE_LOG_DATA(&mve_rsrc_log_fwif3, MVE_LOG_WARNING, vec, 2);

    /* Unmap first page. */
    mve_rsrc_mem_cpu_unmap_page3(inst->desc->data[0]);
}
