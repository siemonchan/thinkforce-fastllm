...
//      confidence prediction for an image.
    ...
    void GetConfidenceScores(const magicType *conf_data, const int num,
                             const int num_preds_per_class, const int num_classes,
                             vector<map<int, vector<float> > > *conf_scores, int filter = -1);
// Get confidence predictions from conf_data.
//    conf_data: num x num_preds_per_class * num_classes blob.
...
//    prior_variances: stores all the variances needed by prior bboxes.
    ...
    void GetOriginalPriorVariances(const magicType* original_prior_data, const int num_priors, vector<vector<magicType>> *original_prior_variances);
